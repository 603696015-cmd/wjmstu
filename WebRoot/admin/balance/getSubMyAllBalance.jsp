...
tr {background-color:expression((this.sectionRowIndex%2==0)?"#ffffff":"#f4f4f4")} 
	...
	<table width="100%" border="0" cellpadding="0" cellspacing="0">
		...
		<form action="getMyAllBalance2.action" method="post"
			name="acc_list">
				<s:hidden name="pN" id="pageNow"/>
				<s:hidden name="pS" />
				<s:hidden name="deptid" />
				<s:hidden name="username" id="username"/>
				<s:hidden name="mybalance" />
				<s:hidden name="balanceValue" id="u_fee"></s:hidden>
				<s:hidden name="userids" id="userids"/>
			<table align="center" cellpadding="1" cellspacing="1" width="100%"
				>
				<tr align="right">
				  <td width="100" bgcolor="#F4F4F4"><a  href="geteluserzengzi_recharge_info1.action?type=2" class="textbg5">余额转移记录</a></td>
				  <td width="120" align="left" bgcolor="#F4F4F4">包含下属部门：<input type="checkbox" name="sub_department"
				  ...
				  id="elUser.realname"></td>
					  <td width="220" align="left" bgcolor="#F4F4F4">账号：					
				  ...
				  id="elUser.username"></td>
					 <td width="190" align="left" bgcolor="#F4F4F4">
				       <input id="find" name="find" type="button" onClick="newsSubmit()" value="搜索" class="textbg6"></td>
			  </tr>
				<tr>
					<%-- 
					<td>
					...
					<s:else><tbody onMouseOut="changeback()" onMouseOver="changeto()" >
						<s:iterator value="elUsers">
							...
							<td height="30" style="padding-left:8px;color:blue;" align="left">
								...
								<td height="30" align="center" >
								   <a href="geteluser_subrecharge_info.action?username= <s:property value="user.id" />" class=textbg4>查 看</a>
								</td>
							</tr>
						</s:iterator></tbody>
					</s:else>
		...
		<script>
			...
			function saveFee(){
				...
				var myb = document.getElementById("mybalance").value;
			...
			if(uid==0){
				...
				if(window.confirm("确定为所选用户每人转移“"+fee1+"”元？")){
					document.getElementById("userids").value=uid;
					document.getElementById("u_fee").value=fee1;
					acc_list.action="allgetMyAllBalance2.action";
					//alert(acc_list.action);
					acc_list.submit();
			...
			<a  onclick="unassign(this)" class="textbg6">批量转移</a>
		  </form>
		</div>
