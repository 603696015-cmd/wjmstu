...
-->
													...
													<s:iterator value="myclasses">
														<tr>
															...
															<s:else>
																<s:if test="canExam==1">
																	<td width="100" align="center" bgcolor="#F8FCFE">
																		<a target="_blank" href="quizpaperinit.action?classid=<s:property value="elClass.id" />&myroom.examroom.id=<s:property value="examRoom.id"/>"><img src="images/xtb021.gif" width="28"
	...
	<script type="text/javascript">
		function needAllocation(){
			...
			var epid = <s:property value="examRoom.examPaper.id" />;
			if(!inDingjiRoom ){
				if(time == 0){
					if(window.confirm("您还未定级,点击确定进行第1次定级考试")){
						enterEroom(<s:property value="roomid" />,epid,0,0,0);
					}
				}
			}else{
		...
		function haha(){
			...
			var time = <s:property value="elUserClassification.time" />;
			if(!inDingjiRoom){
				enterEroom(<s:property value="roomid" />,<s:property value="examRoom.examPaper.id" />,0,0,0);
			}
		}
	</script>
