...
<HTML>
	<HEAD>
		...
		<script type="text/javascript">
			...
			function templateUpload(id,ondemo,tablename){
				if(ondemo == 0){
					alert("该模块还没设置开通自定义模板,不能上传模板!!!");
					return ;
				}
				document.getElementById("clientid").value=id;
				document.getElementById("tablename").value=tablename;
				form_list_client.action="templateUploadInit.action";
				form_list_client.submit();
			}
			
			
			function copy(tablename){
				var url = "addContactTagsInit.action?tablename="+tablename+"&&fromcopy=1";
...
<style type="text/css">
	...
	<BODY><%--  onLoad="initTable();" --%>
		...
		<div style="margin-top: 0px; text-align: center;">
			<!-- 组合查询 -->
			<form action="myModuleManage.action" method="post" name="form_list_client">
				<s:hidden name="pN" id="pageNow" />
				<s:hidden name="moduleManage.id" id="clientid" />
				<s:hidden name="moduleManage.tablename" id="tablename" />
			
			
			<div >
			
				模块名称：	<input name="moduleManage.modulename"   size="40">
				<input type="button" value="搜索"  onClick="search();">
			</div>
			
			</form>
			<br>
			<input type="button" value="查看功能代码详情" onclick="window.location.href='viewFunInformation.action'"/>
				...
				<tbody onMouseOut="changeback()" onMouseOver="changeto()">
					<s:iterator value="list_module">	
						<tr  align="center" >
							...
							<td width="100" height="30" align="center">
								<br>功能代码：myContactTags;<br>参数：tablename=<s:property value="tablename" />&rx=1
							</td>
							<td width="100" height="30" align="center">
								<br>功能代码：finalsearchContactTags;<br>参数：tablename=<s:property value="tablename" />&final_=1
							</td>
							<td width="100" height="30" align="center">
							...
							<td width="200" height="30" align="center">
								...
								<a href="javascript:manage('<s:property value="tablename" />')">字段管理 </a> |
								<a href="javascript:manage('<s:property value="tablename" />')">手机字段管理 </a> |
								<a href="javascript:templateUpload('<s:property value="id" />','<s:property value="ondemo" />','<s:property value="tablename" />')" >模板上传</a> |
								<a href="javascript:copy('<s:property value="tablename" />');" >前台调用地址</a>
							</td>
						</tr>
					</s:iterator>
				</tbody>
		...
		<!-- 
		<div style="float: right; position: absolute;top: 156px;left: 1050px; "><input type="button" value="手机字段管理"/></div>
		 -->
	</BODY>
