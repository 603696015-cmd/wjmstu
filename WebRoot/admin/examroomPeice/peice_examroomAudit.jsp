		...
		<script> 
			...
			function saveFee(){
				...
				if(window.confirm("确定为\""+clname+"\"设定价格为"+fee1+"？")){
					...
					erform.action="peice_audit_examroom.action";
			...
			<table width="100%" cellpadding="1" cellspacing="1">
				<tr>
					<td width="100px" valign="top" id="tree_list_td"
						style="display: ''">
						<wysLib:eroomLibTree
							href="peice_audit_examroom.action?sublibs=1&examRoom.eroomLib.id="
					...
					<td>
						<s:form action="peice_audit_examroom" name="erform"
							theme="simple">
							...
							<table width="100%" height="100%" align="center" cellpadding="1"
								...
								<s:iterator value="examRooms">
									<tr>
										...
										<td align="center" bgcolor="#FFFFFF">
							<s:if test="status!=1">
								<a style="cursor:pointer;"  onClick="sh(<s:property value="examRoom.id"/>, 1);"  class="textbg6">通过</a>
							
							</s:if>
							<s:if test="status==3||status==1">
								<a style="cursor:pointer;"  onClick="sh(<s:property value="examRoom.id"/>, 4);"  class="textbg6">不通过</a>
