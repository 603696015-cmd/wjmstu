...
function checksubmit(){
	//判断submit条件    必须加入字段+必须选择节点
	var tableHTML = $("#showtablename").html();
	var fieldHtml = $("#showfieldname").html();
	if(tableHTML=="" || fieldHtml==""){
		alert("请添加标签要调用的字段");
		return false;
	}
	return true;
...
var rcount=0;
		...
		<form action="lable_lable_cirulationListLableSQLAdd.action"
			name="myform1" method="post" onsubmit="return checksubmit();" >
							...
							<tr>
								<td colspan=3>
									<label>
										...
										<script type="text/javascript">
											var labletreeid = <s:property value="circulationListLable.lableTree.id"/>;
											if(labletreeid == 0){
												w0.setValues([new DEP(1,<s:property value="circulationListLable.lableTree.lid"/>,<s:property value="circulationListLable.lableTree.rid"/>)]);
											}else{
