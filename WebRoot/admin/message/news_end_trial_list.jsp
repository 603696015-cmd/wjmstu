					...
					<s:form action="news_end_trial_list.action" method="post"
						...
						<div style="height:30px;" >
							...
							<select name="news.hot">
								<option value="0" <s:if test="news.hot==0" >selected=selected</s:if>>
									普通
								</option>
								<option value="1" <s:if test="news.hot==1" >selected=selected</s:if>>
									推荐
								</option>
								<option value="2" <s:if test="news.hot==2" >selected=selected</s:if>>
									热门
								</option>
								<option value="3" <s:if test="news.hot==3" >selected=selected</s:if>>
									重点
								</option>
								<option value="4" <s:if test="news.hot==4" >selected=selected</s:if>>
									头条
								</option>
							</select>
							&nbsp;&nbsp;&nbsp;
    ...
    <td><div id="Div_ToolsBar"></div></td>
		...
		<div style="margin-top: 0px; text-align: center;">
			<form action="upNewHot.action" name="myForm" method="post">
				...
				<input name="pN" value="<s:property value="pN" />" type="hidden"/>
				<input name="pS" value="<s:property value="pS" />" type="hidden"/>
