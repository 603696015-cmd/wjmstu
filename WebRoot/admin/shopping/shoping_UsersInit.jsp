...
tr {background-color:expression((this.sectionRowIndex%2==0)?"#ffffff":"#f4f4f4")} 
		...
		<form action="Shopping_user_detail_list.action" method="post" name="course_assignment">
		...
		<s:form action="Shopping_user_detail_list.action" method="post" name="assignSearch_assignment" theme="simple">
			<s:hidden name="course.id" />
			...
			<table width="100%"> 
				...
				<tr>
				   <td> 姓名：<input name="elUser.realname" id="elUser.realname"></td>
				    ...
				    <td><input id="find" name="find" type="submit" value="搜索"></td>
				</tr>
		...
		<table width="100%">
			...
			<%
			   ...
			   String url="Shopping_user_detail_list.action?course.id="+course.getId()+"&deptid=";
			 %>
		...
		<s:if test="myCourses.size==0">当前还没有分配学员</s:if>
			<s:else>
						...
						function unassign(){
						  if(window.confirm("确定取消分配？！")){
						     var checkObj = document.getElementsByName("id");
							  ...
							  course_assignment.action="Shopping_user_detail_unassigntoUsers.action";
			...
			<s:if test="myCourses.size!=0">
				<a href="shopping_course_order.action?course.id=<s:property value="course.id" />" class="textbg">确认订购</a>
			</s:if>
		...
		<form action="Shopping_user_detail_list.action" method="post" name="acc_list">
