...
<HTML>
	<HEAD>
		...
		<script type="text/javascript">
			...
			function del1(basedbId){
				if(window.confirm("确认删除？")){
					document.getElementById("basedbId").value=basedbId;
					basedb.action="eluser_BasedbDel.action?BasedbDel_flag=1";
					basedb.submit();
				}
			}
		...
		/*********************************** TableEdit.js  end *********************************/
			...
			function myload(){
				if('<s:property value="elmessage"/>'!=''&&'${elmessage}'=='success'){
					alert('批量更新成功！');
				}
			}
		</script>
<style type="text/css">
	...
	<body onLoad=" myload()"><%--  onLoad="initTable();" --%>
		...
		<div style="margin-top: 0px; text-align: center;" class="divClass">
			<%-- 
			<table width="500px;" align="center" id="table1">
				<tr>
					<th colspan="5">
						用户字段名称管理（点击对应名称后可修改）
					</th>
				...
				<tr>
					<td height="30" style="padding-left: 8px; color: blue;"
						align="center">
						<s:property value="baseDataTypeList[0].name" />
					</td>
					<td align="center">
						<s:property value="baseDataTypeList[1].name" />
					</td>
					<td align="center">
						<s:property value="baseDataTypeList[2].name" />
					</td>
					<td align="center">
						<s:property value="baseDataTypeList[3].name" />
					</td>
					<td align="center">
						<s:property value="baseDataTypeList[4].name" />
					</td>
				</tr>
			</table>
			 --%>
		...
		<div style="margin-top: 0px; text-align: center;">
			<form action="eluser_BasedbList.action" method="post" name="basedb">
				<s:hidden name="baseDatat.id" id="basedbId" />
				<s:hidden name="pN" id="pageNow" />
				<s:hidden name="pS" />
				<s:hidden name="baseDatat.sortid" id="sortId" />
				<s:hidden name="baseDatat.sortManner" id="sortManner" />
				<s:hidden name="allTypeid" id="allTypeId" />
				<div>
					<%-- 
					<span style="margin-right:50px;"><a href="baseDb_jingzhong_up.action">更新工种数据</a></span>
					 --%>
					<span style="margin-right: 50px;"><a
						href="eluser_addBasedbInit.action" class=textbg>添加字段内容</a>
					</span><a
						href="eluser_BasedbImp.action" class=textbg>批量导入</a> 类别切换&nbsp;
					<%-- 
					<s:select id="selectTypeId" theme="simple" headerKey="-1" headerValue="全部" list="#{1:'工种',2:'职务',3:'职级',4:'岗位',5:'地市'}" name="baseDatat.typeid" value="baseDatat.typeid"/>
					 --%>
					<s:select id="selectTypeId" theme="simple" headerKey="-1"
						headerValue="全部" list="baseDataTypeList" name="baseDatat.typeid"
			...
			<table width="100%" align="center" cellpadding="1" cellspacing="1">
				<tr>
					...
					<s:if test="allTypeid!=1&&count>1&&#session.roleid==1">
						<th width="40"></th>
						<th width="40"></th>
					</s:if>
					<th width="80"></th>
					...
					<s:iterator value="baseDatatList">
						<tr>
							...
							<td width="50" height="30" align="center">
								<s:property value="baseType.name" />
							</td>
							...
							<s:if test="allTypeid!=1&&count>1&&#session.roleid==1">
								<td width="40">
									<s:if test="sortid!=1">
										<a
											href="javascript:sort(1,'<s:property value="sortid" />','<s:property value="typeid" />')">上移</a>
									</s:if>
								</td>
								<td width="40">
									<s:if test="sortid!=count">
										<a
											href="javascript:sort(0,'<s:property value="sortid" />','<s:property value="typeid" />')">下移</a>
									</s:if>
								</td>
							</s:if>
							<td width="120" align="center">
								...
								<a href="javascript:del1('<s:property value="id" />')">直接删除</a>
							</td>
						</tr>
					</s:iterator>
				</tbody>
