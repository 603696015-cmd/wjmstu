	...
	<HEAD>
		...
		<script type="text/javascript">
			function sort(manner,sortid,typeid){
				...
				basedb.action="course_BasedbSort.action";
				basedb.submit();
			}
			function del(basedbId){
				if(window.confirm("确认删除？")){
					document.getElementById("basedbId").value=basedbId;
					basedb.action="course_BasedbDel.action";
					basedb.submit();
				}
			}
			function update(basedbId){
				document.getElementById("basedbId").value=basedbId;
				basedb.action="course_alterBasedbInit.action";
				basedb.submit();
			}
		</script>
		<script type="text/javascript">
		...
		/*********************************** TableEdit.js  end *********************************/
			...
			function updateData(currentRowIndex){
				...
				if(currentRowIndex != null){
					...
					var shihegangwei = rowdata[0];
					var zhuanyeleibie=rowdata[1];
					var zhuanyejibie=rowdata[2]; 
					var shihebumen = rowdata[3];
					var neirongleixing = rowdata[4];
					var peixunleibie = rowdata[5];
					var shihexuewei = rowdata[6];
					var kechengxingzhi = rowdata[7]; 
					//alert("classId:"+ classId + ",suggestcredit:" + suggestcredit +",setcredit:"+setcredit+",getcredit:"+getcredit);
					
					$.post("update_BasedbTypeCourse.action", {
						'colId':currentCell.cellIndex+1,
						'shihegangwei':shihegangwei,
						'zhuanyeleibie':zhuanyeleibie,
						'zhuanyejibie':zhuanyejibie,
						'shihebumen':shihebumen, 
						'neirongleixing':neirongleixing,
						'peixunleibie':peixunleibie,
						'shihexuewei':shihexuewei,
						'kechengxingzhi':kechengxingzhi
						}, 
						function (data) {
							//alert('更新成功');
						});
	...
	<BODY onLoad="initTable();">
		<table width="100%" border="0" cellpadding="0" cellspacing="0">
  			<tr>
    			<td valign="middle" class="tablequiz">
    				<ul class="nav">
			<li>
				<div style="padding-top:3px;color:#077ac7;font-size:12px;"><wysLib:Navigation ivalue="课程基础数据管理" /></div>
			</li>
		</ul>
				</td>
		...
		<div style="margin-top: 0px; text-align: center;" class="divClass">
			<table width="800px;" align="center" id="table1">
				<tr><th colspan="8">课程字段名称管理</th></tr>
				<tr>
					<td height="30" align="center" bgcolor="#ECEDEB"><s:property value="baseCourseTypeList[0].name" /></td>
					<td><s:property value="baseCourseTypeList[1].name" /></td>
					<td><s:property value="baseCourseTypeList[2].name" /></td>
					<td><s:property value="baseCourseTypeList[3].name" /></td>
					<td><s:property value="baseCourseTypeList[4].name" /></td>
					<td><s:property value="baseCourseTypeList[5].name" /></td>
					<td><s:property value="baseCourseTypeList[6].name" /></td>
					<td><s:property value="baseCourseTypeList[7].name" /></td>
				</tr>
			</table>
		</div>
		<div style="margin-top: 0px; text-align: center;" >
			<form action="course_BasedbList.action" method="post" name="basedb">
				<s:hidden name="baseCourse.id" id="basedbId"/>
				<s:hidden name="pN" id="pageNow" />
				<s:hidden name="pS" />
				<s:hidden name="baseCourse.sortid" id="sortId"/>
				<s:hidden name="baseCourse.sortManner" id="sortManner"/>
				<s:hidden name="allTypeid" id="allTypeId" />
				<div>
					<%-- 
					<span style="margin-right:50px;"><a href="baseDb_jingzhong_up.action">更新警种数据</a></span>
					 --%>
					<span style="margin-right:50px;"><a href="course_addBasedbInit.action">添加字段内容</a></span>
					类别切换&nbsp;
					<%-- 
					<s:select id="selectTypeId" theme="simple" headerKey="-1" headerValue="全部" list="#{1:'警种',2:'职务',3:'职级',4:'岗位',5:'地市'}" name="baseDatat.typeid" value="baseDatat.typeid"/>
					 --%>
					 <s:select id="selectTypeId" theme="simple" headerKey="-1" headerValue="全部" list="baseCourseTypeList" name="baseCourse.typeid" listKey="id" listValue="name"/>
			...
			<table width="80%" align="center" cellpadding="1" cellspacing="1"
				bgcolor="#ECEDEB">
				<tr>
					<th width="230" height="30" align="center" bgcolor="#FFFFFF">
						名称					</th>
					
					<th height="30" align="center" bgcolor="#FFFFFF">
						类别
					</th>
					<th height="30" align="center" bgcolor="#FFFFFF">
						描述
					</th>
					<s:if test="allTypeid!=1&&count>1">
					<th></th><th></th>
					</s:if>
					<th></th>
				</tr>
				<tbody onMouseOut="changeback()" onMouseOver="changeto()" >
				<s:iterator value="baseCourseList">
					<tr>
						<td height="30" align="center" bgcolor="#FFFFFF" style="font-weight:bold;">
							<s:property value="basevalue" />
						</td>
						<td height="30" align="center" bgcolor="#FFFFFF">
							<s:property value="baseCourseType.name" />
						</td>
						<td height="30" align="center" bgcolor="#FFFFFF">
							<s:property value="remack" />
						</td>
						<s:if test="allTypeid!=1&&count>1">
						<td>
							<s:if test="sortid!=1">
								<a href="javascript:sort(1,'<s:property value="sortid" />','<s:property value="typeid" />')" >上移</a>
							</s:if>
						</td>
						<td>
							<s:if test="sortid!=count">
								<a href="javascript:sort(0,'<s:property value="sortid" />','<s:property value="typeid" />')" >下移</a>
							</s:if>
						</td>
						</s:if>
						<td>
		  ...
		  <script>
				...
				function initPN(){
					...
					var selectValue=document.getElementById("selectTypeId");
					if(selectValue.value==-1){
						document.getElementById("allTypeId").value=1;
					}else{
						document.getElementById("allTypeId").value=0;
					}
					basedb.submit();
				}
			</script>
