...
<HTML> 
	...
	<table width="100%" border="0" cellpadding="0" cellspacing="0">
		...
		<s:form action="examprac_depInfo" name="epForm" method="post">
			...
			<s:hidden name="statusValue" id="status" value="0" />
		</s:form>
			...
			<div>
				<table width="100%" align="center" cellpadding="1" cellspacing="1"
					...
					<s:iterator value="exampracs">
						<tr>
							...
							<a href="javascript:sh_t('<s:property value="id"/>');" class="textbg4">开 通</a>
							<a href="javascript:sh_nt('<s:property value="id"/>');" class="textbg6">不开通</a>
							<%-- 
							<a target="_blank"
				...
				function sh_p(id){
					if(window.confirm("确定恢复？"))
						document.location.href="examprac_validrecovery.action?examprac.id="+id+"&pageResult=2";
				} 
				function sh_np(id){
					if(window.confirm("确定暂停？")) 
						document.location.href="examprac_validsuspended.action?examprac.id="+id+"&pageResult=2";
				}
				function sh_t(id){
		...
		<s:form action="examprac_del" method="post" name="examprac_del">
			<s:hidden name="pracids" id="pracids" />
		</s:form>
		<s:form action="examprac_validview" method="post" name="page_list" theme="simple">
			<s:hidden name="examprac.id" />
		</s:form>
		<form action="examprac_sh_list.action" method="post" name="erform">
