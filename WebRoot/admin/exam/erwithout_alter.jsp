...
<HTML>
	<HEAD>
		...
		var epcount=parseInt("<s:property value="examRoom.exampapers.size" />");
			...
			*/
			function _onsubmit(){
				var islink = <s:property value="examRoom.islink"/>;
				//if(jzArr.length != 0)
				...
				//验证考频相关参数
				if(document.getElementById("jiangeshijian").value==''){
					alert("间隔时间不能为空");
					return false;
				}
				if(!checkFloat($("#jiangeshijian").val())){
					alert("间隔时间不能为非数字");
					return false;
				}
				if(parseInt($("#jiangeshijian").val())<0){
					alert("间隔时间不能小于0");
					return false;
				}
				var epidsc = 0;
				...
				if(islink==1){
					if(ty==0){
						...
						if($("#invigilators").children().length==0){
							alert("监考员 不能为空，请选择！");
							return false;
						}if($("#appraises").children().length==0){
							alert("阅卷员 不能为空，请选择！");
							return false;
						}
					}
  ...
  function addUserinfo2(_id,id,inputname){
  	...
  	//是否限制考试频次
		function changeIsXianzhi(op){
			var div_xianzhi=document.getElementById("isxianzhikaopin");
			var isxianzhi = document.getElementById("isxianzhi").value;
			if(op==1){
				div_xianzhi.style.display="block";
			}else{
				document.getElementById("examsforday").value='0';
				document.getElementById("jiangeshijian").value='0';
				div_xianzhi.style.display="none";
			}
		}
	...
	<body onLoad="loadData('<s:property value="erRegistration.examRoomIds" />','<s:property value="erRegistration.elclassIds" />');">
		...
		<div style="margin-top:20px;text-align:center;">
			...
			<s:form theme="simple" id="form_exam_add" name="form_exam_add"  
				method="post" action="erwithout_alter"
						...
						<td >
							<label>
								...
								<div id="div_link" style="display: block">  
									...
									<a href=""
										onclick="searchUserInit('selectings','examRoom.selectings.id'); return false;" class="textbg4">添加</a>
								 </div>
						</td>
					</tr>
					 <tr>
						...
						<td >
							...
							<s:if test="examRoom.ertype==1">
							练习考场
							<div id="examCountDiv">
								可考次数：
								<label>
									<input type="text" style="width: 40px;"
										name="examRoom.examcount" id="passgrade" value="<s:property value="examRoom.examcount"/>">
								</label>
							</div>
							</s:if>
							<s:else>
					...
					<tr>
						...
						<td >
							<input name="examRoom.passmanner" type="radio"
								value="1"
								<s:if test="examRoom.passmanner==1">checked="checked"</s:if>
								>
							平均分
							<input name="examRoom.passmanner" type="radio"
								value="0" 
								<s:if test="examRoom.passmanner==0">checked="checked"</s:if>
								>
							最高分
						</td>
					</tr>--%>
					...
					<tr>
						...
						<td >
							<input name="examRoom.markingManner" type="radio"
								value="0" 
								<s:if test="examRoom.markingManner==0">checked</s:if>
							>
							系统自动阅卷
							<input name="examRoom.markingManner" type="radio"
								value="1" 
								<s:if test="examRoom.markingManner==1">checked</s:if>
								>
							手工阅卷
						</td>
					</tr>
				...
				<span style='color:red'>答卷可看</span><input id="b_erdel_<s:property value="#epsst.index"/>" type="checkbox" name="examPapers[<s:property value="#epsst.index"/>].quizlook" <s:if test="quizlook==1">checked='checked'</s:if> <s:if test="status==1">disabled="disabled"</s:if> value="1" />
										...
										<%-- 
										<lable name="prac"></br><span style="color:red;">练习</span>： 
										<span style="width:150px;" id="epts_lx_<s:property value="#epsst.index"/>" ><s:property value="prac.ep_tscore" /></span>
										...
										<a style="cursor: pointer; width: 14px; height: 14px;color:red;" href=""
											onclick="javascript:deleteEps(this,<s:property value="id"/>,0);return false;">X</a>
...
<A style="WIDTH: 14px; FLOAT: right; HEIGHT: 14px; CURSOR: hand" onClick="javascript:deleteExamRoomEpUserinfo(this);return false;" href="javascript:void(0)">X</A>
					...
					<tr>
						...
						<td>
							<input type="radio" value="1" name="examRoom.isxianzhikaopin" 
							<s:if test="examRoom.isxianzhikaopin==1">checked="checked"</s:if>
							id="isxianzhi" onclick="changeIsXianzhi(1);"/>
							是
							<input type="radio" value="0" 
								name="examRoom.isxianzhikaopin" id="isxianzhi"
								<s:if test="examRoom.isxianzhikaopin==0">checked="checked"</s:if>
								  onclick="changeIsXianzhi(0);" />
							否
						</td>
					</tr>
					<tr>
						...
						<td>
						<div id="isxianzhikaopin" <s:if test="examRoom.isxianzhikaopin!=1">style="display:none;"</s:if>>
						<table>
						<tr>
						<td>
						每天考试次数：
						<s:textfield name="examRoom.examsforday" id="examsforday"  />&nbsp;&nbsp;&nbsp;&nbsp;
						每次考试间隔时间：
						<s:textfield name="examRoom.jiangeshijian" id="jiangeshijian"  />小时
						</td>
						</tr>
						</table>
						</div>
					...
					<tr>
						...
						<td >
							...
							<s:if test="examRoom.valid == 5">
								...
								<span style="color: red;">考场已开通,不能再修改考场信息! 如果需要修改。 请申请！</span>
								<br>
								<input type="button" name="" onClick="RoomAlert()"
									value="申请修改信息">
							</s:if>
							<s:else>
