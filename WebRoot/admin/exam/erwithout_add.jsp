...
tr {
	background-color: expression((         this .         sectionRowIndex %         2 ==
		...
		<script type="text/javascript">
			...
			function _onsubmit(){
				...
				if(document.getElementById("islink").checked){  //在线考场
					if(ty==0){
				...
				if(document.getElementById("examsforday").value==''){
					alert("考试次数不能为空");
					return false;
				}
				if(!checkFloat($("#examsforday").val())){
					alert("考试次数不能为非数字");
					return false;
				}
				if(parseInt($("#examsforday").val())<0){
					alert("考试次数不能小于0");
					return false;
				}
				...
				if(document.getElementById("islink").checked){
					if(epidsc<=0){
						alert("请添加试卷");
						return false;
					}
  ...
  function checkHasUser(_id,id){
	...
	function show(va){
		document.getElementById("tupian").style.display=va;
		document.getElementById("jiankao").style.display=va;
		document.getElementById("bangding").style.display=va;
		document.getElementById("xianzhi").style.display=va;
		document.getElementById("mima").style.display=va;
		document.getElementById("yuejuan").style.display=va;
		document.getElementById("fuhe").style.display=va;
		document.getElementById("shijuan").style.display=va;
		document.getElementById("tianjia").style.display=va;
		document.getElementById("shenqing").style.display=va;
			...
			<s:form id="examroom_add" name="form_exam_add" method="post"
				...
				<table width="100%" align="center" cellpadding="1" cellspacing="1"
					...
					<tr>
						...
						<td>
							<input name="examRoom.islink" id="islink" type="radio" value="1" checked="checked" onclick="return show('block');">
							在线考场
							<input name="examRoom.islink" type="radio" value="0" onclick="return show('none');">
							线下考场
						</td>
					</tr>
					<tr id="tupian">
					...
					<tr>
						...
						<td>
							<input type="radio" value="1" name="examRoom.isxianzhikaopin" id="isxianzhi" onclick="changeIsXianzhi(1);"/>
							是
							<input type="radio" value="0" checked="checked"
								name="examRoom.isxianzhikaopin" id="isxianzhi"  onclick="changeIsXianzhi(0);" />
							否
						</td>
					...
					<tr>
						...
						<td>
						<div id="isxianzhikaopin" style=" display: none;">
						<table>
						<tr>
						<td>
						每天考试次数：
						<s:textfield name="examRoom.examsforday" id="examsforday" value="0" />&nbsp;&nbsp;&nbsp;&nbsp;
						每次考试间隔时间：
						<s:textfield name="examRoom.jiangeshijian" id="jiangeshijian" value="0" />小时
						</td>
						</tr>
						</table>
						</div>
