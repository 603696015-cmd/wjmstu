...
tr {background-color:expression((this.sectionRowIndex%2==0)?"#ffffff":"#f4f4f4")} 
		...
		<table width="100%">
			<tr>
				...
				<td valign="top">
					<s:form action="elclass_assignlist2" name="myclist" theme="simple">
				...
				<td width="80" align="center" >
					<s:property value="classSize"/>	  
									...
									<s:if test="status == 0 || status == 2"> 
										<a href="elclass_assign2userInit.action?sub_department=1&elclass.id=<s:property value="id" />" class="textbg">人员增减</a>		
										<!-- <a href="elclass_assign2depInit.action?elclass.id=<s:property value="id" />" class="textbg">分配部门</a> -->
										<a style="cursor:pointer;"  onClick="sh(<s:property value="id"/>, 1);" class="textbg">复核通过</a>  
									</s:if><s:else>									
										 <a href="elclass_details_sh.action?elclassId=<s:property value="id" />&PageStatus=0&PageStatusint=2" class=textbg>详情查看</a>
										<a href="elclass_check_students.action?sub_department=1&elclass.id=<s:property value="id"/>" class="textbg">查看学员</a>
									</s:else>
								</td>
								</tr>
							</s:iterator></tbody>
							...
							<form action="elclass_sh.action" name="elclass_sh" method="post">
								...
								<s:hidden name="Return" id="Return" value="elclass_uvalidlist"></s:hidden>
							</form>  
							...
							<script>   
								function sh(id,status){
								    ...
								    document.getElementById("status").value=status; 
								 	if(status==1 && window.confirm("确定申请审核？")){
