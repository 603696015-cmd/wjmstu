		...
		<table width="100%" border="0" cellpadding="0" cellspacing="0">
  			<tr>
    			<td valign="middle" class="tablequiz">
    				<ul class="nav">
			<li>
				<div style="padding-top:3px;color:#077ac7;font-size:12px;"><wysLib:Navigation ivalue="考场详情页" /></div>
			</li>
			<!--<li>
		...
		<div style="margin-top: 0px; text-align: center;">
			...
			<table width="90%" align="center" cellpadding="1" cellspacing="1"
				...
				<tr>
					...
					<td >
						<font color="<s:if test="examRoom.uvalid==1">green</s:if>">
							<s:property value="examRoom.uvalidName" />
						</font>
						...
						<script type="text/javascript">
								function sh_p(){
									if(window.confirm("确定让它通过审核？"))
										document.location.href="examroom_sh_p.action?examRoom.id=<s:property value="examRoom.id"/>"
								}
								function sh_np(){
									if(window.confirm("确定让它不通过审核？"))
										document.location.href="examroom_sh_np.action?examRoom.id=<s:property value="examRoom.id"/>"
								}
								function page(i){
					...
					<s:if test="examRoom.valid == 0">
						 <s:if test="examRoom.uvalid==1">
							...
							<input style="height:35px;" class="textbg6" type="button" name="button2" onClick="sh_np();"	id="button2" value="不通过审核" />
						 	...
						 	<span style="color:red">人员还未审核，请先审核人员！</span>
						 </s:else>
					</s:if> <s:else>
						<span style="color:red">考场已审核！</span>
					</s:else>
					</td>
				</tr>
			</table>
