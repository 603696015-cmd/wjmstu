		...
		<table width="100%" cellpadding="1" cellspacing="1">
			<tr>
				<td width="165" valign="top" id="tree_list_td" style="display:none">
					<wysLib:clTypeTree  href="elclass_applyAlter_list.action?sublibs=1&cltype.id="
						rootAble="true" />
			  </td>
				...
				<td valign="top"> 
					...
					<s:form action="elclass_applyAlter_list" name="myclist" theme="simple">
						<s:hidden name="pN" id="pageNow"></s:hidden>
						<s:hidden name="pS"></s:hidden>
						<s:hidden name="cltype.id"></s:hidden>
                        <table width="100%" border="0" cellpadding="0" cellspacing="1" bgcolor="#D1E4F5">
  <tr>
    <td width="85" rowspan="2" bgcolor="#F8FCFE"><div style="text-align: left;" id="showtree">
									<a href="javascript:showtree(true);" class="textbg5">显示类别</a>
								</div></td>
    <td width="500" bgcolor="#F8FCFE">包含下级节点: 
    ...
    <td rowspan="2" bgcolor="#F8FCFE">&nbsp;<s:submit cssClass="textbg4" value="搜索"></s:submit></td>
    </tr>
  <tr>
					...
					<s:else>
						<table width="100%" height="100%" align="center" cellpadding="1"
							...
							<s:iterator value="elclasses" status="elclasses_st" >
								<tr <s:if test="(#elclasses_st.index%2)==1">style="background:#f4f4f4"</s:if> >
								  ...
								  <td align="center" >
									...
									<td align="left" >  
											<a href="elclass_details_sh.action?elclassId=<s:property value="id" />&PageStatus=5&PageStatusint=5&Return=aal" class="textbg4">详 情</a>
											<a href="elclass_check_students.action?elclass.id=<s:property value="id"/>&sub_department=1&Return=aal" class="textbg4">学 员</a>	
											<s:if test="status == 5">
											<a style="cursor:pointer;"  onClick="sh(<s:property value="id"/>,6,<s:property value="status" />
											);" class="textbg6">申请修改</a>    
											<a href="CRE_notelistInit.action?elclass.id=<s:property value="id"/>&Return=elclass_applyAlter_list" onclick="showCre(<s:property value="id"/>);return false;" class="textbg4">备 注</a>											</s:if>								  </td>
									<!-- <td width="60" align="center" >
											<a href="elclass_check_students.action?elclass.id=<s:property value="id"/>" class="textbg6">查看学员</a>			
										  <a onClick="return confirm('确定删除该培训班？')" 
										href="elclass_delete_sh.action?elclass.id=<s:property value="id"/>&pN=0&pS=10" class="textbg4">删 除</a>
									</td>   -->
								</tr>
								<s:if test="examRooms.size!=0">
									<tr <s:if test="(#elclasses_st.index%2)==1">style="background:#f4f4f4"</s:if>>
											...
											<s:if test="examRooms.size!=0">
												<s:iterator value="examRooms">
													...
													<s:if test="valid==0||valid==2">
														<a style="cursor:pointer;"  onClick="shEroom(<s:property value="id"/>, 1,'falsa','falsa','true','elclass_applyAlter_list');" class="textbg6">创建完成</a>
													</s:if>
													<s:if test="valid!=5">
							...
							<form action="elclass_sh.action" name="elclass_sh" method="post">
								...
								<s:hidden name="astatus" id="astatus"></s:hidden>
								<s:hidden name="Return" id="Return" value="elclass_applyAlter_list"></s:hidden>
							</form> 
		<div style="text-align: center;margin-bottom: 20px;"><wysLib:page></wysLib:page></div>
							<script>   
								function sh(id,status,astatus){
								    document.getElementById("elclass.id").value=id;
								    document.getElementById("status").value=status; 
								    document.getElementById("astatus").value=astatus; 
