...
tr {background-color:expression((this.sectionRowIndex%2==0)?"#ffffff":"#f4f4f4")} 
		...
		<!-- 内容 --> 
		<iframe id="xuanxiuFrame" src="elclass_details.action?elclassId=${elclass.id}" width=100% height=280 
			marginwidth="0" marginheight="0" frameborder=0  onload="this.height=xuanxiuFrame.document.body.scrollHeight + 20"></iframe>
		<div style="margin-top: 0px; text-align: center;">
		<form action="elclass_delete_sh_NOInit.action" method="post" name="assignSearch_assignment">
		...
		<table width="100%">
			...
			<%
			   ...
			   String url="elclass_delete_sh_NOInit.action?elclass.id="+elclass.getId()+"&deptid=";
			 %>
		...
		<s:if test="elusers.size==0">当前还没有分配学员</s:if>
			...
			<form action="elclass_delete_sh_NOInit.action" method="post" name="course_assignment">
	     ...
	     <s:form action="elclass_delete_sh_NO" method="post" theme="simple" name="sh">  
		     <table width="100%">
			    <!--  <tr>
						<td height="30" style="padding-left:8px;color:blue;" align="left">
					  不通过标题：
			     ...
			     <tr>
					...
					<td height="30" align="center" >
					  <s:if test="ecAudit.content != null">
						<textarea name="ecAudit.content" cols="60" rows="7" disabled="disabled">
						 <s:property value="ecAudit.content"/></textarea> <br>
					</s:if>
					<s:textarea name="ecAudit.replycontent" cols="60" rows="7"></s:textarea> 
					<s:hidden name="elclass.id" />     
					</td>
				</tr>
			     <tr>
		...
		<!-- 内容 --> 
				...
				function sh_p(){
					...
					sh.action="elclass_delete_sh_NO.action?ecAudit.status=1";
					sh.submit(); 
				}
				function sh_np(){
					if(window.confirm("确定让它不通过审核？"))
					sh.action="elclass_delete_sh_NO.action?ecAudit.status=4";
					sh.submit(); 
				}
				function page(i) {
		...
		<form action="elclass_delete_sh_NOInit.action" method="post" name="acc_list">
