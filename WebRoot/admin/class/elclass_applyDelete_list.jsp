		...
		<table width="100%" cellpadding="1" cellspacing="1">
			<tr>
				<td width="165" valign="top" id="tree_list_td" style="display:none">
					<wysLib:clTypeTree
						href="elclass_applyDelete_list.action?sublibs=1&cltype.id="
						rootAble="true" />
				</td>
				...
				<td valign="top">
					...
					<s:form action="elclass_applyDelete_list" name="myclist"
						theme="simple">
  ...
  <tr>
    <td bgcolor="#F8FCFE">时间段范围&nbsp;&nbsp;&nbsp;从&nbsp;&nbsp;&nbsp;&nbsp;
								...
								<s:iterator value="elclasses" status="elclasses_st" >
									<tr <s:if test="(#elclasses_st.index%2)==1">style="background:#f4f4f4"</s:if> >
									   ...
									   <s:else>
										...
										<td align="left">
											<a
												href="elclass_details_sh.action?elclassId=<s:property value="id" />&PageStatus=-1&PageStatusint=-1&Return=adl"
												class="textbg4">详 情</a>
											<s:if test="status != 8">
												<a style="cursor: pointer;"
													onClick="sh(<s:property value="id"/>,8,<s:property value="status"/>);"
													class="textbg6">申请删除</a>
												<a
													href="CRE_notelistInit.action?elclass.id=<s:property value="id"/>&Return=elclass_applyDelete_list"
													onclick="showCre(<s:property value="id"/>);return false;" class="textbg4">备 注</a>
											</s:if>
											<a
												href="elclass_check_students.action?elclass.id=<s:property value="id"/>&sub_department=1&Return=adl"
												class="textbg4">学 员</a>
										</td>
											...
											<s:if test="examRooms.size!=0">
												<s:iterator value="examRooms">
													...
													<s:if test="valid==0||valid==2">
														<a style="cursor:pointer;"  onClick="shEroom(<s:property value="id"/>, 1,'falsa','falsa','true','elclass_applyDelete_list');" class="textbg6">创建完成</a>
													</s:if>
													<s:if test="valid!=5">
		...
		<form action="elclass_sh.action" name="elclass_sh" method="post">
			...
			<s:hidden name="Return" id="Return" value="elclass_applyDelete_list"></s:hidden>
		</form>
		<script>   
								function sh(id,status,astatus){
								    ...
								    document.getElementById("astatus").value=astatus; 
								 	if(status==8 && window.confirm("确定申请删除？")){
