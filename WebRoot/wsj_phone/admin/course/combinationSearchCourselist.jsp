		...
		<script>
			function sh(id,status){
			    ...
			    document.getElementById("status").value=status; 
			 	if(window.confirm("确定要还原到申请审核之前的状态？")){
		...
		<div style="margin-top: 0px; text-align: center;">
				...
				<tr>
					<td width="200px" valign="top" id="tree_list_td">
						<wysLib:ctypeTree rootAble="true"
							href="combinationSearchCourselist.action?str=combinationSearchCourse&course.status=-1&course.ctype.id="></wysLib:ctypeTree>
					</td>
					...
					<td valign="top">
						<s:form action="combinationSearchCourselist" name="myclist"
							theme="simple">
							<s:hidden name="pN" id="pageNow"></s:hidden>
							<s:hidden name="pS"></s:hidden>
							<s:hidden name="course.status" value="-1"></s:hidden>
							<s:hidden name="course.ctype.id"></s:hidden>
				课程名称：<s:textfield name="course.name"></s:textfield>
							<%--  <s:submit value="搜索"></s:submit> --%>
							<input type="button" value="搜索" class="textbg4"
								onClick="courseSeash();" />
						</s:form>
										...
										<s:iterator value="courses">
											...
											-->
												...
												<td align="left">
													<%-- 
													<a
														href="course_user_detail_list.action?course.id=<s:property value="id"/>"
														class="textbg">查看学员</a>
													 --%>
													<s:if test="status==0">
														<a href="course_view.action?course.id=<s:property value="id"/>&Return=csc"
															class="textbg">课件制作</a>													</s:if>
													<s:else>
														<a
															href="course_preview.action?course.id=<s:property value="id"/>"
															target="_blank " class=textbg>预览</a>
														<a href="course_view.action?course.id=<s:property value="id"/>&Return=csc"
															class="textbg">课件制作</a>
													</s:else>
													<s:if test="status==9">
			...
			<form action="course_sh.action" name="course_sh" method="post">
				...
				<s:hidden name="Return" id="Return"
					value="combinationSearchCourselist"></s:hidden>
			</form>
			<wysLib:page></wysLib:page>
			<a href="combinationSearchCourseInit.action" class="textbg4"
