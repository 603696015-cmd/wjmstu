...
<html>
	<head>
		...
		<base href="<%=basePath%>" />
		<title>预览课程(scorm1.2)--<s:property value="course.name" />
		</title>
...
function toPreOrNext(x){
	if(x==1)
		document.location="course_scorm_catalog.action?course.id=<s:property value="course.id"/>&scormcourse.navitype=next&scormcourse.nowScoid=<s:property value="scormcourse.scoid"/>"
	else
		document.location="course_scorm_catalog.action?course.id=<s:property value="course.id"/>&scormcourse.navitype=prev&scormcourse.nowScoid=<s:property value="scormcourse.scoid"/>"
}
		...
		<ul class="menu" id="catalog">
			<s:iterator value="scormcourse.scoList">
				<li>
					...
					<s:else>
						<a href="course_scorm_catalog.action?course.id=<s:property value="course.id"/>&scormcourse.scoid=<s:property value="scoid"/>"><s:property
