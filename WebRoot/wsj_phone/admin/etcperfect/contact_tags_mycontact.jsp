...
<HTML>
	<HEAD>
		...
		<script type="text/javascript" src="js/newversion/mycontacttags.js"></script>
		<script type="text/javascript">
		...
		<script type="text/javascript">
			...
			function view(id){//alert(id);
					...
					//form_list_client.action="viewContactTags.action";
					form_list_client.action="viewContactTags.action";
					form_list_client.submit();
			}
			function view_ZDY(id){//alert(id);
					document.getElementById("contactid").value=id;
					form_list_client.action="viewContactTagsZDY.action";
					form_list_client.submit();
			}
			function del(ids){
			...
			function update(id,actionName){
				...
				//form_list_client.action="updateContactTagsInit.action";
				form_list_client.action="updateContactTagsInit.action";
				form_list_client.submit();
			}
			function update_ZDY(id,actionName){
				...
				document.getElementById("actionName").value=actionName;
				//document.getElementById("ismodify").value="modify";
				form_list_client.action="updateContactTagsInitZDY.action";
				form_list_client.submit();
			}
			function commitBySelf(ids){
				if(window.confirm("确认提交审核？")){
					document.getElementById("contactids").value=ids;
					form_list_client.action="commitverifypassContactTags.action";
					form_list_client.submit();
				}
			}
			...
			function apply_update(ids){//alert(id);
					if(window.confirm("确认申请修改?")){
						document.getElementById("contactids").value=ids;
						form_list_client.action="myaddApplyUpate.action";
						form_list_client.submit();
					}
			}
			function apply_del(ids){//alert(id);
				if(window.confirm("确认申请删除?")){
					document.getElementById("contactids").value=ids;
					form_list_client.action="myaddApplyDel.action";
					form_list_client.submit();
				}
			}
		...
		/*********************************** TableEdit.js  end *********************************/
			...
			function show_qiuhe(){
				...
				$.each(array,function(i,arr){
					var columnName = "";
					var result = "";
					if(array[i].is_qiuhe == 1){//求和
						columnName = array[i].columnName;
						$.ajax({
						  type: 'POST',
						  url: "getQiuheResult.action",
						  data: {columnName:columnName,tablename:tablename},
						  async:false,//同步
						  success: function(data){
					  		data = eval("("+data+")");
					  		result = data.check_json_result;
						  }
						});
						
						//共有多少个table列，并且求和列为那一列
						var str = "<tr><td align='center'>总价</td>";
						for(var j=0;j<array.length + 2;j++){
							if(j < array.length){
								if(array[j].columnName == columnName){
									str += "<td align='center'>"+Math.round(new Number(result) * 100) / 100+"</td>";
								}else{
									str += "</td><td>";
								}
							}else{
						...
						str += "</tr>";
						$(str).appendTo($("#last_tr"));;
						//$("<tr><td align='center'>总价</td><td></td><td></td><td></td><td align='center'>"+Math.round(new Number(result) * 100) / 100+"</td><td></td><td></td></tr>").appendTo($("#last_tr"));
					}
				});
			}
			...
			function load(){
				if("${type}" == "1"){
					...
					//alert(window.opener.document.body.clientWidth);
					window.raload();
...
<style type="text/css">
	...
	<BODY onLoad="opensearchdiv();load();add_info();"><%--  onLoad="initTable();" --%>
		...
		<div style="margin-top: 0px; text-align: center;">
			<!-- 组合查询 -->
			<form action="myContactTags.action" method="post" name="form_list_client">
				...
				<s:hidden name="fromActionName" ></s:hidden>
