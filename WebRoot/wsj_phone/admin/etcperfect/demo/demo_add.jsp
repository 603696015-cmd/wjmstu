...
<html>
		...
		function addRelate(returnvalue,type,id){
			...
			$.ajax({
				  ...
				  success: function(data){
				  		...
				  		$.each(data,function(i,obj){
							...
							//alert(produce_columns);
				  			for(var jj = 0;jj<produce_columns.length;jj++){
				  				...
				  				/**
				  				if(type == 1){
				  					value = getColumnValueByAuto("<s:property value='tablename'/>",produce_columns[jj].split("_-_")[2],produce_columns[jj].split("_-_")[0],ids_array[i]);
				  					tr += "<td align='center' ><span  id='"+produce_columns[jj].split("_-_")[2]+"=="+produce_columns[jj].split("_-_")[0]+"=="+i+"' >"+value+"</span></td>" ;
						...
						/**
						for(var u=0;u<ppp.length;u++){
							alert(document.getElementById(ppp[u].split("_-_")[2]+"=="+ppp[u].split("_-_")[0]+"=="+0).innerHTML);
						}
						*/
				  }
			});
  ...
  <wysLib:TBHTML_JAVASCRIPT itype="add" />
  <body onLoad="myload();">
		...
		<div style="margin-top:0px;"> 
			<s:form action="addContactTags" method="post" name="lineTrainRecord_add"
					...
					<!-- 
						说明:
							...
							<tr>
								...
								<td align="center">
									<wysLib:TBHTML_ADD iname="xxx"></wysLib:TBHTML_ADD>
								</td>
							</tr>
