	...
	<BODY>
		...
		<script type="text/javascript">
 				$(function(){
 					...
 					$("#temp").attr("tmpId", '<s:property value="columnTmp.tmpId"/>').attr("selected","selected"); 
 				});
				function close(){
		...
		<div style="margin-top: 40px; text-align: left; margin-left: 20px;">
			<form name="message_management" id="message_management"
				action="column_alterTemp.action" method="post">
				<input type="hidden" name="columnTmp.id" value="<s:property value="columnTmp.id"/>" />  
				<label id="modelIdLabel">
					栏目名:<s:if test="columnTmp.columnType =='XW'">
									新闻--->
								</s:if>
					 ...
					 <s:property value="columnTmp.columnName"/>  
				</label>
				<br/>  
				<label>
					模板
					 <select id="temp" name="columnTmp.tmpName">
					 	<s:iterator value="templateList">
					 		<option tmpId="<s:property value="id"/>" value="<s:property value="id"/>-=lwh=-<s:property value="name"/>-=lwh=-<s:property value="jspTmp"/>"><s:property value="name"/></option>
					 	</s:iterator> 
					 </select>
				</label>
				<br/>   
