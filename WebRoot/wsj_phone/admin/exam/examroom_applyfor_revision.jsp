			...
			<table width="100%" cellpadding="1" cellspacing="1">
				<tr>
					<td valign="top" id="tree_list_td" style="display:none">
						<wysLib:eroomLibTree
							href="examroom_applyfor_revision.action?sublibs=1&str=eroomlib&eroomLib.id="
					...
					<td>
						<form
							action="examroom_applyfor_revision.action?eroomLib.id=<s:property value="eroomLib.id"/>"
			  ...
			  <div>								 
									...
									<tr>
										...
										<td width="120" height="30" align="center">
											...
											<s:if test="isApplication == 1">
												<br/><span style="color: red">计划：<s:property
														value="planNumber" /> </span>
											</s:if>
										</td>
										<td width="180" height="30" align="left">
											<a style="cursor: pointer;"
												onClick="sh(<s:property value="id"/>, 6);" class="textbg6">申请修改</a>
											<a
												href="CRE_notelistInit.action?examRoom.id=<s:property value="id"/>&Return=examroom_applyfor_revision"
											...
											<!--	<a href="examroom_sh_view.action?examRoom.id=<s:property value="id"/>" class="textbg4">查 看</a> 	
												<a onClick="return confirm('确定不通过审核？')" href="examroom_sh_np.action?examRoom.id=<s:property value="id"/>&examRoom.valid=2" class="textbg6">不通过</a> 
											<s:else> 
												<s:if test="avalid == 1">
													<a href="examroom_modify_throughthe.action?examRoom.id=<s:property value="id"/>&examRoom.avalid=0" onclick="return confirm('确定让申请修改通过？')" class="textbg6">修改申请通过</a>
													<a href="examroom_modify_throughthe.action?examRoom.id=<s:property value="id"/>&examRoom.avalid=2" onclick="return confirm('确定让申请修改通过？')" class="textbg6">不通过</a>
												</s:if>
											</s:else>	 -->
										</td>
									</tr>
						...
						<form action="examroom_sh_p.action" name="examroom_sh_p"
							...
							<s:hidden name="Return" id="Return"
								value="examroom_applyfor_revision"></s:hidden>
						...
						<script>   
								function sh(id,valid){
								    ...
								    document.getElementById("examRoomValid").value=valid; 
								 	if(valid==6 && window.confirm("确定申请修改？")){
								...
								function FillInNoteksInit(id){ 
									 ...
									 var rv =  window.showModalDialog("CRE_addNotes.action?examRoom.id="+id+"&course.id=0&elclass.id=0&Return=elclass_primash_list&x="+Math.random(),null,sFeature); 
									 if(rv == null){
								 ...
								 function showCre(roomid){
									 ...
									 height=500;  
								  	 var sFeature="dialogWidth:"+width+"px;dialogHeight:"+height+"px;Status:0;resizable:1;help:0";
									 var rv =  window.showModalDialog("CRE_notelistInit.action?examRoom.id="+roomid+"&Return=examroom_prima_shlist&x="+Math.random(),null,sFeature); 
								}	
							</script>
					</td>
				</tr>
