...
<table width="100%" border="0" cellpadding="0" cellspacing="0">
			...
			<li>
				<span style="font-weight: bold;">课程定价审核</span>
			</li>
		</ul>
				</td>
		...
		<table width="100%" >
			<tr>
				<td width="200px" valign="top" id="tree_list_td"><wysLib:ctypeTree rootAble="true" href="peice_AuditList.action?ctype.id=" ></wysLib:ctypeTree></td>
				<td valign="middle" width="5px;" style="padding: 0px" >
							<img src="images/leftmenu/main_55.gif" style="cursor: hand"
										onclick="changeTreeDisplay(this)" />
					</td>
			<td valign="top">
			<s:form action="peice_AuditList" name="myclist" theme="simple">
				<s:hidden name="pN" id = "pageNow"></s:hidden>
				<s:hidden name="pS"></s:hidden>
				<s:hidden name="ctype.id"></s:hidden>
				<s:hidden name="setstatus"  id="status"></s:hidden>
				<s:hidden name="course.id" id="c_id"></s:hidden>
				<s:hidden name="wpeice" id="c_fee"></s:hidden>
				<s:hidden name="pt" id="c_type"></s:hidden> 
				<s:hidden name="ifadmin" ></s:hidden> 
				
				<s:select theme="simple"  headerValue="全部" headerKey="0"
									list="#{1:'未定价',4:'已定价',2:'待审核定价',3:'已审核定价'}"
									name="stype" value="stype" />
				课程名称：<s:textfield name="course.name" id="testname"></s:textfield> <input type="button"  value="搜索" onClick="newsSubmit()" />
			...
			<table width="100%" height="100%" align="center" cellpadding="1"
				...
				<s:iterator value="cp">
					<tr>
						...
						<td align="center">
							<a style="cursor:pointer;"  href ="course_preview.action?course.id=<s:property value="course.id"/>"  class="textbg6"  target="_blank" >浏览</a>
						</td>
							
						
						<td align="center" bgcolor="#FFFFFF">
							<s:if test="status!=1">
								<a style="cursor:pointer;"  onClick="sh(<s:property value="course.id"/>, 1,<s:property value="course.eroom.id"/>);"  class="textbg6">通过</a>
							
							</s:if>
							<s:if test="status==3||status==1">
								<a style="cursor:pointer;"  onClick="sh(<s:property value="course.id"/>, 4,1);"  class="textbg6">不通过</a>
		...
		<input type="button" onClick=" document.getElementById('fee').style.display='none'" value="关闭"/>
			...
			function saveFee(){
				...
				if(window.confirm("确定为\""+cname+"\"设定价格为"+fee1+"？")){
					...
					myclist.action="peice_AuditList.action";
				...
				function sh(id,status,room){
									if(room==0){
										if(window.confirm("该课程未设置考场，确定要通过？")){
