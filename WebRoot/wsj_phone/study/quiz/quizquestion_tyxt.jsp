...
<html xmlns="http://www.w3.org/1999/xhtml">
	<head>
		...
		<link href="images/tyxt_images/css.css" type="text/css"
			rel="stylesheet" />
...
function playQmusic(answer){
	var iAnswer = parseInt(answer.split("-=SpEl=-")[0]);
	if(right.URL!=""){
		right.controls.play();	
		 //做一个循环去判断 第一个视频是否结束
	    state = setTimeout("checkState(1,"+iAnswer+")", 1000);	
	}
}
function playQmusic2(answer){
	var iAnswer = parseInt(answer.split("-=SpEl=-")[0]);
	if(error.URL!=""){
		error.controls.play();	
	  //做一个循环去判断 第一个视频是否结束
	  state = setTimeout("checkState(2,"+iAnswer+")", 1000);	
	}
...
function getObject(index){
 ...
 //答错后，播放错误提示音，错误提示音播完后，播放选择图片的音频文件
  ...
  var music_statu = null;
   function checkState(type,iAnswer){
   		if(type == 1){
   				if(right.playState==1||right.playState==8){
				   clearTimeout(state);
				   getObject(iAnswer).controls.play();
				   music_statu = setTimeout("checkMusicState("+type+","+iAnswer+")", 1000);
			   }else{
				   setTimeout("checkState("+type+","+iAnswer+")", 1000);
			   }
   		}else if(type == 2){
   ...
   function checkMusicState(type,iAnswer){
	   if(getObject(iAnswer).playState==1||getObject(iAnswer).playState==8){
		   ...
		   setTimeout("checkMusicState("+type+","+iAnswer+")", 1000);
	   }
   }
			   	...
			   	if(data=='success'){
					...
					if(jdata.atime==1)
						if(jdata.status==1){//1次回答正确
							...
							playQmusic(jdata.answer);
						}else{
							//继续作答
							//alert("对不起，回答错误，请继续作答");
							playQmusic2(jdata.answer);
						}
				...
				<div id="m_z">
					...
					<div id="m_zz">
						<table width="100%" border="0" cellspacing="0" cellpadding="0">
							<tr>
								<td align="center" id="ques_title_images">
									${question.title }
								</td>
								<!-- 
								<td height="400" align="center">
									<img src="images/shij.jpg" width="220" height="176" alt="pic_0" onclick="setVal(0);" />
								</td>
								<td align="center">
									<img src="images/shij.jpg" width="220" height="179" alt="pic_1" onclick="setVal(1);" />
								</td>
								<td align="center">
									<img src="images/shij.jpg" width="220" height="179" alt="pic_2" onclick="setVal(2);" />
								</td>
								 -->
							</tr>
						</table>
								...
								<table border="0" cellspacing="0" cellpadding="0">
									<tr>
										<s:if test="view==0"><!-- 考试 -->
											...
											<td>
												<input type="button" value="" onclick="submittyxt()"
													style="cursor: pointer; background-image: url(images/tyxt_images/bf/tj.png); width: 64px; height: 50px;" />
											</td>
											<td height="79" width="74">
