...
tr {
	...
	<table width="100%" border="0" cellpadding="0" cellspacing="0">
			...
			<table width="100%" cellpadding="2" cellspacing="1">
				...
				<tr height="26">
					...
					<td style="padding-left: 10px;" colspan="2">
						全部有
						<s:property value="#request.class_all" />
						个， 已结业
						<s:property value="#request.class_yes" />
						个
					</td>
					<td style="padding-left: 10px;" width="80">
						考场:
					...
					<td style="padding-left: 10px;" colspan="2">
						全部有
						<s:property value="#request.eroom_all" />
						个， 已通过
						<s:property value="#request.eroom_ok" />
						个
					</td>
				</tr>
			</table>
					...
					<s:iterator value="myClasses">
						<tr>
							...
							<s:if test="passed">
								...
								<td align="center">
									<a target="_blank"
										href="diploma_view.action?elclass.id=<s:property value="elClass.id"/>&elUser.id=<s:property value="elUser.id"/>"
										class=textbg4>查 看</a>
								</td>
							</s:if>
							<s:else>
							...
							<td align="center">
								<a
									href="statisticStudyLearnLocus.action?elUser.id=<s:property value="elUser.id"/>&course.classid=<s:property value="elClass.id"/>&Return=studyOverviewInfo.action?elUser.id=<s:property value="elUser.id"/>"
									class=textbg4>查 看</a>
							</td>
						</tr>
					</s:iterator>
		...
		<div style="margin-top: 0px;">
				...
				<tbody onMouseOut="changeback()" onMouseOver="changeto()">
					<s:iterator value="myrooms">
						<tr>
							...
							<td height="30" align="center">
								<a
									href='lookStudyEroom.action?myroom.examroom.id=<s:property value="examroom.id"/>&elUser.id=<s:property value="elUser.id"/>&Return=studyOverviewInfo.action?elUser.id=<s:property value="elUser.id"/>'
			...
			<a href="user_searchlist.action" class="textbg">返回列表</a>
			</div>
		</div>
