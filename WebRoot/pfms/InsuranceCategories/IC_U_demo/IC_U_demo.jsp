...
<html>
	<head>
		...
		<link rel="stylesheet" type="text/css" href="pfms/InsuranceCategories/IC_U_demo/${IC.tableName}.css" />
		...
		function getInformationByAuto(){
				...
				$.ajax({
					 type: 'POST',
					 url: "getInformationByAuto.action",
			...
			function search_user_information(tablename,type){
				if(type == "user"){
					width=800;
					height=600;
					var url = "searchRelateId.action?x="+Math.random()+"&tablename="+tablename;
	...
	<BODY onLoad="myload();">
		...
		<s:form action="IC_U_addOrAlter" method="post" name="IC_U_addinfo" theme="simple" > 
		...
		<!-- 内容区域结束 -->
						...
						<!--<s:if test="IC.read_auto_biaodi.length()>0 || IC.read_auto_toubaoren.length()>0 || IC.read_auto_beibaoren.length()>0">
							<input type="button" value="自动读取" onclick="getInformationByAuto();" class="textbg6"/>
						</s:if>-->
		...
		<!--<s:if test="IC.read_auto_biaodi.length()>0 || IC.read_auto_toubaoren.length()>0 || IC.read_auto_beibaoren.length()>0">
			<center>
				<input type="hidden" name="id" id="shebei_id"/>
				<input type="hidden" name="tablename" id="tablename" value="<s:property value='IC.tableName'/>"/>
	    		...
	    		<input type="hidden" name="IC.id" id="IC.id" value="<s:property value='IC.id'/>"/>
