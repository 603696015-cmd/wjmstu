...
// ***************************************************************** 
function Folder(folderDescription, hreference,right_target) //constructor 
{ 
  //constant data 
...
{ 
  ...
  if (this.level>0) 
    if (this.isLastNode){ //the last child in the children array 
	    if(this.level==1)
			leftSide = leftSide + "<td align=left class='tree_bar_unselected'>" + auxEv + "<span name='nodeIcon" + this.id + "' id='nodeIcon" + this.id + "'></span></a></td>"
		else{
			var w=25*(this.level-1);
			leftSide = leftSide + "<td align=left width='"+w+"'>" + auxEv + "<span name='nodeIcon" + this.id + "' id='nodeIcon" + this.id + "'></span></a></td>"
		}
	}
	else {
		if(this.level==1){
			leftSide = leftSide + "<td valign=middle class='tree_bar_unselected' align=left height='30px;'>" + auxEv + "<span name='nodeIcon" + this.id + "' id='nodeIcon" + this.id + "'></span></a></td>"
		}
		else{
			var w=25*(this.level-1);
			leftSide = leftSide + "<td align=left width='"+w+"'>" + auxEv + "<span name='nodeIcon" + this.id + "' id='nodeIcon" + this.id + "'></span></a></td>"
  ...
  docW = this.blockStartHTML("folder");
   if (this.level==1) 
	  docW = docW + "<tr>" + leftSide + "<td class='tree_bar_unselected_middle' width='10px;' align='left'>";
   else
	  docW = docW + "<tr>" + leftSide + "<td width='10px;' align='left'>";
  if (USEICONS)
  {
  ...
  if (WRAPTEXT)
	  docW = docW + "</td>"+this.prependHTML+"<td valign=middle>"
  else{
	  if(this.level==1)
		 docW = docW + "</td>"+this.prependHTML+"<td valign=middle nowrap  class='tree_bar_unselected_middle'>"
	  else
		 docW = docW + "</td>"+this.prependHTML+"<td valign=middle nowrap  >"
  }
  if (USETEXTLINKS) 
  { 
    docW = docW + this.linkHTML(true) 
	docW = docW +"<span style='cursor:hand;' onClick='javascript:clickOnNode(\""+this.getID()+"\")'>&nbsp;"+ this.desc + "</span></a>"
  } 
  else 
    docW = docW + this.desc
  docW = docW + "</td>"
  if(this.level==1)
	docW = docW + "<td class='tree_bar_unselected_right'>&nbsp;</td>"
  else
	docW = docW + "<td >&nbsp;</td>"
  ...
  {
	  if (supportsDeferral) {
		  doc.write("<span id=domRoot></span>") //transition between regular flow HTML, and node-insert DOM DHTML
...
{ 
  ...
  { 
	if (USEFRAMES)
	  docW = docW + "<a href='" + this.hreference + "' TARGET=\""+ this.right_target +"\" "
	else
...
{ 
  ...
  if (this.level>0) 
    ...
    { 
      leftSide = leftSide + "<td valign=middle>&nbsp;</td>"
    } 
    else 
    { 
      leftSide = leftSide + "<td valign=middle>&nbsp;</td>"
  ...
  if (USEICONS)
      docW = docW + "<a " + fullLink  + " id=\"itemIconLink"+this.id+"\">" + "<span id='itemIcon"+this.id+"'/>" + "</a>"
  else
	  if (this.prependHTML == "")
...
function blockStartHTML(idprefix) {
  ...
  if (browserVersion == 2) {
    docW = "<layer "+ idParam + " top=0 visibility=show>"
  }
  else if (browserVersion != 0){
    docW = "<table " + idParam + " style='display:block; position:block;' border=0 cellspacing=0 cellpadding=0 width=100%><tr><td>"
  }
...
function leftSideHTML(leftSideCoded) {
	...
	for (i=0; i<leftSideCoded.length; i++)
	{
		...
		{
			retStr = retStr + "<td valign=middle ></td>"
		}
...
alert(obj.xID);
	for(var i = 0;i<obj.children.length;i++){
		var  obji= obj.children;
		id = obji[i].xID;
		fObi = findObj(id);
		if (typeof fObi=='undefined' || fObi==null)
  		{
 		}else{
			fObi.setState(true) ;
			clickOnNodeObj(fObi);
...
function gFld(description, hreference,right_target) 
{ 
  folder = new Folder(description, hreference,right_target);
  return folder;
} 
...
{ 
  ...
  foldersTree.initialize(0, false, "") 
