...
var gLoaded = false;		  //页面是否全部加载完毕，包括所有图片和文字。   gLoaded的属性在window.onload = function(){……}执行后变为true	
var ev = null;
...
/*
 * 把页面上的指定iframe成为可编辑模式
 * 这个方法在window.onload后被调用
 */
...
document.onmousemove = function(e){
	if(gIsIE) var el = event.srcElement;         //srcElement  对于生成事件的 Window 对象、Document 对象或 Element 对象的引用。
