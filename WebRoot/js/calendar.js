document.write("<div align='center' id=meizzCalendarLayer style='padding:3px;Z-INDEX: 999999; BACKGROUND: #63A3E9; LEFT: 205px; WIDTH: 180px; LINE-HEIGHT: 5px;  POSITION: absolute; TOP: 220px; HEIGHT: 215px; display: none'>");
document.write("<iframe name=meizzCalendarIframe scrolling=no frameborder=0 width=100% height=100%></iframe></div>");
function writeIframe()
{
    var strIframe = "<html><head><meta http-equiv='Content-Type' content='text/html; charset=UTF-8'><style>"+
    "*{font-size: 12px; font-family: 宋体}.bg{font-weight:bolder;}.out{width:25px;}#tableDay .op{font-weight:normal;background:#ffffff;}"+
    "#tableDay .op span{margin:0px;padding:0px;}#tableDay{background:#63A3E9} #tableDay td{font-weight:bolder;text-align:center;cursor: pointer;}"+
    "</style></HEAD><body onselectstart='return false' style='margin: 0px;padding:0px' oncontextmenu='return false'><form name=meizz>";
    ...
    strIframe += "<select name=tmpYearSelect onblur='parent.hiddenSelect(this)' style='z-index:1;position:absolute;top:4;left:33;display:none'"+
    " onchange='parent.WebCalendar.thisYear =this.value; parent.hiddenSelect(this); parent.writeCalendar();'></select>"+
    "<select name=tmpMonthSelect onblur='parent.hiddenSelect(this)' style='z-index:1; position:absolute;top:4;left:97;display:none'"+
    " onchange='parent.WebCalendar.thisMonth=this.value; parent.hiddenSelect(this); parent.writeCalendar();'></select>"+
    "<select name=tmpHourSelect onblur='parent.hiddenSelect(this)' style='z-index:1; position:absolute;top:188;left:33;display:none'"+
    " onchange='parent.WebCalendar.thisHour=this.value; parent.hiddenSelect(this); parent.writeCalendar();'></select>"+
    "<select name=tmpMinuteSelect onblur='parent.hiddenSelect(this)' style='z-index:1; position:absolute;top:188;left:75;display:none'"+
    " onchange='parent.WebCalendar.thisMinute=this.value; parent.hiddenSelect(this); parent.writeCalendar();'></select>"+
    "<select name=tmpSecondSelect onblur='parent.hiddenSelect(this)' style='z-index:1; position:absolute;top:188;left:113;display:none'"+
    " onchange='parent.WebCalendar.thisSecond=this.value; parent.hiddenSelect(this); parent.writeCalendar();'></select>"+
    "<table id=tableMain border='0' cellpadding='1' cellspacing='1'  bordercolor='#63A3E9' bgcolor='#63A3E9' width='100%' height='100%'>"+
    ...
    "    <td width=60 id=meizzYearHead title='点击此处选择年份' style='cursor: hand'  onclick='parent.funYearSelect(parseInt(this.innerText, 10))'"+
    "        onmouseover='this.bgColor=parent.WebCalendar.darkColor; this.style.color=parent.WebCalendar.lightColor'"+
    "        onmouseout='this.bgColor=parent.WebCalendar.lightColor; this.style.color=parent.WebCalendar.wordColor'></td>"+
    "    <td width=50 id=meizzYearMonth title='点击此处选择月份' style='cursor: hand'  onclick='parent.funMonthSelect(parseInt(this.innerText, 10))'"+
    "        onmouseover='this.bgColor=parent.WebCalendar.darkColor; this.style.color=parent.WebCalendar.lightColor'"+
    "        onmouseout='this.bgColor=parent.WebCalendar.lightColor; this.style.color=parent.WebCalendar.wordColor'></td>"+
    ...
    "    <table id=tableDay height='100%' width='98%' border=0 cellspacing=1 cellpadding=1>";
         for(var x=0; x<5; x++){ strIframe += "<tr>";
         for(var y=0; y<7; y++) strIframe += "<td class=out id='meizzDay"+ (x*7+y) +"'></td>"; strIframe += "</tr>";}
         strIframe += "<tr>";
         for(var x=35; x<38; x++) strIframe += "<td class=out id='meizzDay"+ x +"'></td>";
         strIframe +="<td class='op' colspan=4><span title=\"设置时间后请点确定\" onclick='parent.calcfm()'>确定</span> <span title=\"当前日期时间\" onclick='parent.setNow()'>当前</span> <span onclick='parent.hiddenCalendar()' title=\"关闭日历\">关闭</span></td></tr></table>"+
    ...
    "        <td width=60 id=meizzHourHead title='点击此处选择小时' style='cursor: hand'  onclick='parent.funHourSelect(parseInt(this.innerText, 10))'"+
    "        onmouseover='this.bgColor=parent.WebCalendar.darkColor; this.style.color=parent.WebCalendar.lightColor'"+
    "        onmouseout='this.bgColor=parent.WebCalendar.lightColor; this.style.color=parent.WebCalendar.wordColor'></td>"+
    "    <td width=50 id=meizzMinuteHead title='点击此处选择分钟' style='cursor: hand'  onclick='parent.funMinuteSelect(parseInt(this.innerText, 10))'"+
    "        onmouseover='this.bgColor=parent.WebCalendar.darkColor; this.style.color=parent.WebCalendar.lightColor'"+
    "        onmouseout='this.bgColor=parent.WebCalendar.lightColor; this.style.color=parent.WebCalendar.wordColor'></td>"+
    "    <td width=50 id=meizzSecondHead title='点击此处选择秒' style='cursor: hand'  onclick='parent.funSecondSelect(parseInt(this.innerText, 10))'"+
    "        onmouseover='this.bgColor=parent.WebCalendar.darkColor; this.style.color=parent.WebCalendar.lightColor'"+
    "        onmouseout='this.bgColor=parent.WebCalendar.lightColor; this.style.color=parent.WebCalendar.wordColor'></td>"+
...
function funYearSelect() //年份的下拉框
{
    ...
    var min = 1911;//(y - n >= 1000) ? y - n : 1000;
    var max = 2062//(y + n <= 9999) ? y + n : 9999;
...
function setNow(){
    ...
    {
    	var d = new Date();
  		WebCalendar.objExport.value =  d.getFullYear()+"-"+appendZero(d.getMonth()+1)+"-"+appendZero(d.getDate())
  		+" "+ appendZero(d.getHours()) +":"+ appendZero(d.getMinutes()) +":"+ appendZero(d.getSeconds());
  		  WebCalendar.thisYear   = d.getFullYear();
          WebCalendar.thisMonth = d.getMonth()+1;
          WebCalendar.thisDay    = d.getDate();
          WebCalendar.thisHour  = d.getHours();
          WebCalendar.thisMinute =d.getMinutes();
          WebCalendar.thisSecond    = d.getSeconds();
...
function calcfm(){
    ...
    {
        var returnValue;
        //var a = (arguments.length==0) ? WebCalendar.thisDay : arguments[0].split("/");
        var d = WebCalendar.format.match(/^(\w{4})(-|\/)(\w{1,2})\2(\w{1,2})$/);
        if(d==null){alert("你设定的日期输出格式不对！\r\n\r\n请重新定义 WebCalendar.format ！"); return false;}
        var flag = d[3].length==2 || d[4].length==2; //判断返回的日期格式是否要补零
        returnValue = flag ? WebCalendar.thisYear +d[2]+ appendZero(WebCalendar.thisMonth) +d[2]+ appendZero(WebCalendar.thisDay) :WebCalendar.thisYear +d[2]+ WebCalendar.thisMonth +d[2]+ WebCalendar.thisDay;
