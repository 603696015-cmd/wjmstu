...
QuizPaper.prototype.autosave=function(){
	...
	setIT = window.setInterval(function(){ 
		...
		if(status==2||status==3){
			jjAlert("试卷已被强制交卷,Exampaper has been submitted by force");
			//alert("试卷已被强制交卷,Exampaper has been submitted by force");
		...
		if(lefttime<=0){
	 		window.clearInterval(setIT);
	 		setIT = null;
	 		jjAlert("时间到，强制交卷,Time is up");
			//alert("时间到，强制交卷,Time is up");
		 	obj.submit();
		}
		obj.passt++;
	},1000);
...
QuizPaper.prototype.submit=function(){
	...
	if(window.opener){
		//window.opener.unblockUser();
		//window.opener.parent.unblockUser();
...
function q_show_help(bsortid){
	var bid = $("#ca_block_"+bsortid).attr("blockid");
	var nowqtype = $("#ca_block_"+bsortid).attr("qtype");
	if(nowqtype==19||nowqtype==20||nowqtype==18||nowqtype==15||nowqtype==16||nowqtype==17){
			...
			"quizblockhelpinit.action?myExamPaper.id="+sqid_
			+"&epblock.id="+bid
			+"&epblock.sortid="+bsortid);
	}
}
...
function q_show(bsort,sort){
	...
	if(nowqtype==8||nowqtype==9||nowqtype==10||nowqtype==19||nowqtype==20||nowqtype==18||nowqtype==15||nowqtype==16||nowqtype==17){
		...
		{
			alert("本题已做答，不能再答,Do not answer again");
		}
	}
	    ...
	    (
	      {
	           ...
	           success: function (json,status )
               {
               		...
               		jjAlert("请输入文件Please enter the file");
...
function intelligentProportion(){
	var qid = $("#ca_question_"+nowbsort+"_"+nowsort).attr("qid");
	var qtype = $("#ca_question_"+nowbsort+"_"+nowsort).attr("qtype");
	$.ajax({
		async:false,    
		type:"post",
