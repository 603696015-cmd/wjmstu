...
dTree.prototype.node = function (node, nodeId) {
	...
	var strname = node.name.split("-x--x-")[0];
	var strcheck=node.name.split("-x--x-")[1];
	var checkname=node.name.split("-x--x-")[2];
	
	if(node.id!=0){
		if(node.name.split("-x--x-").length>1)
			str+="<input type='checkbox' name='"+checkname+"' id=\"ch" + this.obj + nodeId + "\" value='"+strcheck+"'>"
	}
	if (node.url) {
