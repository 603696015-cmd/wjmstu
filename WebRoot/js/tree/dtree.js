...
// Tree object
function dTree(inputtype,inputname,ivalue,objName) { 
...
dTree.prototype.node = function (node, nodeId) {
	...
	if (this.config.useIcons) {
		...
		if(this.inputtype=="ra_f"&&node.id>0&&node.ischk!=-3){
			if(this.ivalue==node.id){
				str+="<input type=\"radio\" style=\"margin:0px;\" name=\""+this.inputname+
				"\" value=\""+node.id+"\" checked=\"checked\"/> ";
			}else{
		...
		}else if(node.ischk==-3){
			if(this.ivalue==node.id){
				str+="<input type=\"radio\" disabled=\"disabled\" style=\"margin:0px;\" name=\""+this.inputname+
				"\" value=\""+node.id+"\" checked=\"checked\"/> ";
			}else{
				str+="<input type=\"radio\" disabled=\"disabled\" style=\"margin:0px;\" name=\""+this.inputname+
				"\" value=\""+node.id+"\" /> ";
		...
		//alert(node.id); 
		if(this.inputtype=="cb_2"&&node.id>=-1){  
			m++;
			if(m==1){
				/*
...
function onGrant12(){
	...
	if(isK12==0){
		d12.openAll();
		document.getElementById("btnK12").value="全部关闭";
		isK12=1;
	}else{
		d12.closeAll();
		document.getElementById("btnK12").value="全部展开";
		isK12=0;
	}
}
