...
dTree.prototype.node = function (node, nodeId) {
	...
	if(node.id!=0){
		...
		*/
		if(strcheck=='true'){
			str+="<input title=\""+node.treeType+node.id+"_"+node.pid+"\" id=\""+node.treeType+node.id+"\" type=\"checkbox\" value='"+node.id+"' style=\"margin:0px;\" name=\"role.funcs.id\" value=\""+node.id+"\" onclick='selectEd(\""+node.treeType+"\",this.title,1);' checked=\"checked\" /> ";
		}else{
			str+="<input title=\""+node.treeType+node.id+"_"+node.pid+"\" id=\""+node.treeType+node.id+"\" type=\"checkbox\" value='"+node.id+"' style=\"margin:0px;\" name=\"role.funcs.id\" value=\""+node.id+"\" onclick='selectEd(\""+node.treeType+"\",this.title,1);' /> ";
		}
	}
	if (node.url) {
