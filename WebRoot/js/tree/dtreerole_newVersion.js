...
function Node(id, pid, name, url,ischk,treeType, title, target, icon, iconOpen, open) {
	this.id = id;				//Ã¨ÂÂÃ§ÂÂ¹id
	this.pid = pid;				//pid
	this.name = name;			//Ã¨ÂÂÃ§ÂÂ¹Ã¥ÂÂ
	this.url = url;				//Ã¨ÂÂÃ§ÂÂ¹Ã©ÂÂ¾Ã¦ÂÂ¥
	this.ischk=ischk;			//Ã¥ÂÂ¤Ã¦ÂÂ­Ã¦ÂÂ¯Ã¥ÂÂ¦ Ã©ÂÂÃ¤Â¸Â­Ã¥Â¤ÂÃ©ÂÂÃ¦Â¡Â
	this.treeType=treeType;
	this.title = title;			//Ã¨ÂÂÃ§ÂÂ¹Ã¦Â ÂÃ©Â¢Â
	this.target = target;
	this.icon = icon;
	this.iconOpen = iconOpen;
	this._io = open || false;
...
dTree.prototype.node = function (node, nodeId) {
	...
	var strname = node.name.split("===")[0];
	var strcheck=node.name.split("===")[1];
	if(node.id!=0){
		/*
		if(strcheck=='true')
		str+="<input type='checkbox' checked='checked' onclick='selectParents("+node.id+")' name='role.funcs.id' id=\"ch" + this.obj + nodeId + "\" value='"+node.id+"'>"
		else
		str+="<input type='checkbox' onclick='selectParents("+node.id+")' name='role.funcs.id' id=\"ch" + this.obj + nodeId + "\" value='"+node.id+"'>"
		*/
		if(strcheck=='true'){
			str+="<input title=\""+node.treeType+node.id+"_"+node.pid+"\" id=\""+node.treeType+node.id+"\" type=\"radio\" value='"+node.id+"' style=\"margin:0px;\" name=\"role.funcs.id\" value=\""+node.id+"\" onclick='selectEd(\""+node.treeType+"\",this.title,1);'  /> ";
		}else{
			str+="<input title=\""+node.treeType+node.id+"_"+node.pid+"\" id=\""+node.treeType+node.id+"\" type=\"radio\" value='"+node.id+"' style=\"margin:0px;\" name=\"role.funcs.id\" value=\""+node.id+"\" onclick='selectEd(\""+node.treeType+"\",this.title,1);' /> ";
		}
	}
...
function selectEd(treeType,id,v){
	...
	var ss = can_selected(oid_oid);
	if(ss==parseInt(0)){
		alert("您只能选择有权限的菜单!");
		document.getElementById(treeType+oid_oid+"").checked = false;
		return ;
	}
	/**
	...
	for(var i=0;i<array.length;i++){
		if(array[i].type=="radio"&&ts.test(id+"")){
			qid=array[i].title;
			
			qid_qid=qid.substr(qid.lastIndexOf("_")+1,qid.length);
			//qid_oid=qid.substr(4,oid.indexOf("_")-4);
			qid_oid=qid.substring(4,qid.lastIndexOf("_"));
			if(qid_oid==oid_qid&&v==1){
				document.getElementById(treeType+oid_qid).checked="checked";
				selectEd_3(treeType,array[i].title,1);
			}
			...
			if(qid_qid==oid_oid){
				...
				selectEd(treeType,array[i].title,0);
			}
		}
	}
	*/
...
function selectEd_3(treeType,id,v){
	var qid=id;
	var oid=id;
	var qid_oid;
	var qid_qid;
	...
	for(var i=0;i<array.length;i++){
		...
		/*	if(qid_qid==oid_oid){//Ã¦ÂÂ¾Ã¥ÂÂ°Ã¦ÂÂÃ¦ÂÂpidÃ¨ÂÂÃ§ÂÂ¹
				...
				selectEd_2(array[i].title,0);//Ã©ÂÂÃ¥Â½ÂÃ¥Â¾ÂªÃ§ÂÂ¯Ã¥ÂÂ°Ã¦ÂÂÃ¦ÂÂÃ¥Â­ÂÃ¨ÂÂÃ§ÂÂ¹
...
function selectEd_2(treeType,id,v){
	...
	for(var i=0;i<array.length;i++){
		if(array[i].type=="radio"&&ts.test(id+"")){
			//Ã¦ÂÂ¾Ã¥ÂÂ°qlibÃ©ÂÂÃ¥ÂÂÃ¤ÂºÂ
			qid=array[i].title;
			
			qid_qid=qid.substr(qid.lastIndexOf("_")+1,qid.length);
			//Ã¥Â¾ÂÃ¥ÂÂ°obj.pidÃ¦ÂÂÃ¥Â¯Â¹Ã¥ÂºÂÃ§ÂÂÃ§ÂÂ¶id
			qid_oid=qid.substr(4,oid.indexOf("_")-4);
			if(qid_oid==oid_qid&&v==1){
				//Ã¦ÂÂ¾Ã¥ÂÂ°Ã¤ÂºÂÃ¦Â­Â¤Ã¨ÂÂÃ§ÂÂ¹Ã§ÂÂÃ§ÂÂ¶Ã¨ÂÂÃ§ÂÂ¹
...
function can_selected(id){
	var ii=0;
	$.ajax(
	 {	    async:false,  
			type:"post",   
		    url:"can_selected.action",   
