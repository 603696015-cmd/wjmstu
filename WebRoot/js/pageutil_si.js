function getPageDiv(cnt,pn,ps,funcname){
var page_div=$("<div>");
			var count = cnt;
			var pageNow = pn;
			var pageSize = ps;
			...
			var xx="";
			if (pageNow > 0) {
				xx+=("<a style='cursor: hand' href='javascript:"+funcname+"("
						+ (0) + ")'>[首页]</a>");
				xx+=("<a style='cursor: hand' href='javascript:"+funcname+"("
						+ (pageNow - 1) + ")'>[上一页]</a>");
			} else {
			...
			if (pageCount > 0) {
				xx+=("<select  onchange='"+funcname+"(this.options[this.selectedIndex].value)'>");
