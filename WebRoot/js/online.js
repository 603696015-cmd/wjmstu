...
function changeText() {
	...
	getObj('spanSpeed').value = Math.round(getObj('spanRate').value / time * 60) ;
 }
	...
	function init_(){
 	...
 	function start_() {
		...
		autoRun1 = window.setInterval(changeText, 1000);
 	}
 	...
 	function timer() {
 		...
 		getObj('spanTime').value = time;//str + (time - fen * 60) ;
 		var txt = getObj('txtInput').value;
 		txt = txt.replace(/\r\n/g, '\n');
 		getObj('spanSpeed').value = Math.round(getObj('spanRate').value/ time * 60) ;
 	}
