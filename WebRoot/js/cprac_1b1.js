...
QuizPaper.prototype.autosave=function(){
	...
	setIt = window.setInterval(	function(){ 
		...
		if((obj.during - obj.passtime -obj.nowpassed )<=0&&(obj.during_js - obj.passtime_js-obj.nowpassed_js)<=0){
			window.clearInterval(this.setIt);
		}
...
QuizPaper.prototype.settime_js=function(){
	...
	var leftsec1 = this.during - this.passtime-this.nowpassed;
	var sec = leftsec%60;
	var min = parseInt(leftsec/60)%60;
	var hour = parseInt(leftsec/60/60);
	var xxx = hour;
	...
	//$("#"+this.timeinput_js).attr("value", this.passtime_js+this.nowpassed_js);
