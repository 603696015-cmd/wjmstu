...
function restuserBox(uname,domain){
	...
	$("#unameShowboxformobile").hide();
	$("#unameShowboxformobileall").hide();
...
function showuserCheckbox(type,uname,domain,data){
	...
	if(type=="numberstart"){
		...
		if(domain=="163.com"){
			...
			$("#unameShowboxformobile").show();
			
			$("#continuemobile").attr("href","http://reg.email.163.com/mailregAll/reg0.jsp?version=mobilereg&mobile="+uname);
			
			//禁用
			$("#passwInp").attr("disabled","disabled");
			$("#passConfim").attr("disabled","disabled");
			$("#verifyInp").attr("disabled","disabled");
			$("#mobileInp").attr("disabled","disabled");
			$("#serviceItemChek").attr("disabled","disabled");
			$("#regBtn").attr("disabled","disabled");
			
			$("#vcodeitem").attr("class","g-collection-item g-collection-item-disable");
			$("#passworditem").attr("class","g-collection-item g-collection-item-disable");
			$("#usernotice").attr("class","g-collection-item g-collection-item-disable");
			$("#passwordconfitem").attr("class","g-collection-item g-collection-item-disable");
			$("#mobileitem").attr("class","g-collection-item g-collection-item-disable");
		}else{
			
			$("#continuemobileotherdomain").attr("href","http://reg.email.163.com/mailregAll/reg0.jsp?version=mobilereg&mobile="+uname);
			//禁用
			$("#passwInp").attr("disabled","disabled");
			$("#passConfim").attr("disabled","disabled");
			$("#verifyInp").attr("disabled","disabled");
			...
			$("#unameShowboxformobileall").show();
			$("#otherdomaincontent").empty();
			$("#otherdomaincontent").append(uname+"<strong class=\"txt-succ\">@163.com</strong> <b class=\"ico ico-free\"></b>");
...
function changeDomain(me){
	...
	RegResult.domain=me.value;
	 var myDate=new Date() ;
	//当为yeah时，显示英文验证码
	if(RegResult.domain=="yeah.net"||RegResult.domain=="126.com"){
	...
	$("#imageA").click();
	//解禁用
	$("#passwInp").removeAttr("disabled");
	$("#passConfim").removeAttr("disabled");
	$("#verifyInp").removeAttr("disabled");
	$("#mobileInp").removeAttr("disabled");
	$("#serviceItemChek").removeAttr("disabled");
	$("#regBtn").removeAttr("disabled");
	
	$("#vcodeitem").attr("class","g-collection-item");
	$("#passworditem").attr("class","g-collection-item");
	$("#passwordconfitem").attr("class","g-collection-item");
	$("#mobileitem").attr("class","g-collection-item");
	$("#usernotice").attr("class","g-collection-item");
...
function chkMobilevmobile(){
	...
	if(/^13\d{9}$/.test( mobile ) | /^15\d{9}$/.test( mobile ) | /^18\d{9}$/.test( mobile )|/^14\d{9}$/.test( mobile ) ){
		...
		//判断是否为保留账号。
		var params={
				ajaxurlwithparam:"checkmobile.do?mobile="+mobile+"&random="+Math.random(),
				method:"GET",
				data:""
		};
		var checkresult=ajaxRequest(params);
		if(checkresult.message==1){ //是保留账号
			showNoticeMessage("wrong","该邮箱帐号已存在","mobileWrap");
			return;
		}
        ...
        var hosturl="mbind.mail.126.com";
        requestData.type = "text/javascript";  
        requestData.src = "http://"+hosturl+"/mbind/qu.do?pn="+mobile;
	...
	if(/^([a-z]|[A-Z])[0-9a-zA-Z_]+$/.test( mobile ))
		return 2;
	return -1;
}
 ...
 * @returns {Number}
 */
function chkAuthcodeformobile(){
	var authcode=$.trim($("#vcode").val());
	if(authcode=="") 
		return 0;
	else 
		return 1;
