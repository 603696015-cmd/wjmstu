...
QuizPaper.prototype.autosave=function(){
	...
	setIt = window.setInterval(	function(){ 
		var lefttime = obj.timer.leftsec;//obj.during+obj.during_js - obj.passtime -obj.nowpassed ;
			if(obj.timer.nowpassed%30==29){
				//obj.getstatus();
				$("#"+this.timeinput).attr("value", obj.timer.passtime+obj.timer.nowpassed);
			}
		if(status==2||status==3){
			alert("试卷已被强制交卷!");
			window.clearInterval(this.setIt);
...
QuizPaper.prototype.submit=function(){
	$("#" + this.theform).attr("action","courseprac_submit.action");
...
function fq_show(){
	var q = now_q;
		...
		if(q.qtype==8||q.qtype==9||q.qtype==10){
			...
			$("#oprate_question").attr("src","cpracquestioninit.action?myExamPaper.id="+sqid_+"&question.epblock.id="+q.block.id+"&question.id="+q.id);
			q.oldclass='ca_td_yd';
...
//	document.getElementById("a_office_"+blockid+"_"+qid).value =document.getElementById("a_office_"+blockid+"_"+qid).value+filename2.substring(filename2.lastIndexOf('.'));
	$.ajaxFileUpload
	    (
	      {
	      	   url:"", //你处理上传文件的服务端upload_office_stuff.action
	           secureuri:false,
