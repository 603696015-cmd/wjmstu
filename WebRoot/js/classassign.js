...
function assign(){
	 if(window.confirm("确定分配？！")){
	    ...
	    if(toc==0){
			course_assignment.action="elclass_newassign2user_add.action";
...
function seachOnClassPage(pn){
	$("#queryManner").val(0);
 	...
 	var pa1 = $("#sousuo").serialize();
	$.post("elclass_seachUser_class.action", pa1+"&x="+Math.random(), function (data) {
		if(data=='err1'){
			alert("没有选择培训班");
		}else{
			var jsondata = eval("("+data+")");
			...
			for(var i = 0;i<ulist.length;i++){
				...
				var x = '<input type="checkbox" name="id" value="'+ulist[i].id+':'+ulist[i].assign+'" />';
	...
	function classWriteUser(id){
		document.location.href="classWriteUserInit.action?elClass.id="+id;
	}