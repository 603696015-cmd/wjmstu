...
function setPassTime() {
	...
	function saveQuizPaper(){
		/*quizform.action="quizpaper_save.action";
		document.getElementById("passTime").value= passTime ;
		quizform.submit();*/
		var pa1 =getFormData();
		action("quizpaper_save.action", pa1 ,"save3" );
		alert("保存成功！");
	}
	function submitQuizPaper(){
		quizform.action="questionnaire_quizpaper_submit.action?myExamPaper.id=<s:property value="myExamPaper.id"/>";
		quizform.submit();
	}
</SCRIPT>
			...
			<form action="quiz_submit.action" method="post" name="quizform"
				id="exam_show" escapeAmp="false">
				<wysLib:quizPaper2></wysLib:quizPaper2>
...
//=============ajax封装结束========================================
	...
	function addParam(name, value) {
        /*dataString += (dataString.length > 0 ? "&" : "")
            ...
            dataString += (dataString.length > 0 ? "&" : "")+name+"="+value;
    }
