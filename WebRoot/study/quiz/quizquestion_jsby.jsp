...
.tbox{position:absolute; display:none; padding:10px; background:#ffffff url(image/preload.gif) no-repeat 50% 50%; border:10px solid #e3e3e3; z-index:2000;overflow-y:auto;}
...
TINY.box.show({html:content3,boxid:'frameless',width:550,height:300,fixed:false,maskid:'bluemask',maskopacity:40,closejs:function(){closeJS(obj1)}});
			...
			function closeJS(obj){
				
			obj.image.src = "images/bf/anniu_31.png";
		obj.video.Play();
   ...
   function checkVideo(type,atime,wrongCount){
	   debugger;
   		if(type == 0 && atime == 0){//初次加载，依次播放两段视频，播放完成后，加载倒计时
			...
			vedio_state = null;
			vedio_state = setTimeout("checkFirstVideoIsPlayOver("+type+","+atime+","+wrongCount+")",1000);
	    }else if(type == 1 && atime == 1){//第一次答对，系统播放答对提示音+完整对话文
		checkVideoflag=1;
	   		//暂停正在播放的音频或者视频
	   	    js_stop();
...
//20141021 角色扮演标注	
var jsbyRemark_=-1;
var sortid__='<s:property value="question.sortid" />';
var stuffImage='<s:property value="question.epblock.cosPlayRemark" />'
$(function(){
	...
	//}
		//20141218语音解析
	//	uploadRecord_=setInterval("uploadRecord()", 1000);
		
	//	checkupload_=setInterval("checkupload()", 1000);
		
});

function jsbyRemark(){
	if(jsbyRemark_==-1){
			setTimeout("replaceImg()",400);
		}else{
			init_statu = setTimeout("checkInit()",1000);
		}
...
function replaceImg(){
	...
	document.getElementById("swfcontent").innerHTML='<img src="http://www.fhse.net/wjm/'+stuffImage+'"/>';
	jsbyRemark_=0;
	//jsbyRemark();
}
...
function submitjsby(){
	...
	if(ifview == 0){
		if(endtime == 0){//时间到
			...
			if(isfuting==1&&ansflag==1){
				document.getElementById("answerflag").value=5;
				isfuting=0;
			}
				var xx = $('#theform').formSerialize();
				$.ajax({
				...
				$.alerts.dialogClass = "style_1";
	   			jConfirm("该题还没有进行录音，您确定提交吗?question has not be recoding,Submit,Yes or No?", 'Information', function(r) {
					$.alerts.dialogClass = null; // reset to default
					if(r){
						document.getElementById("answerflag").value=2;
						var xx = $('#theform').formSerialize();
						$.ajax({
							beforeSend:function(){
								submitstatus = 0;
...
function js_stop(){
	var flashvideo = getFlashMovieObject("video");
	var image = document.getElementById("pause_play");
	image.src = "images/bf/anniu_36.png";
	flashvideo.Rewind();
}
...
function userspeak(){
	...
	var questionid = <s:property value="question.id" />;
	var userid = <s:property value="#session.userId" />;
	var name = "c:/record/"+userid+"_"+sqid+"_"+questionid+".wma";
	speak.URL = name;
	if(name){
		speak.controls.play();
	}else{
...
function blockUser(message){
  ...
  var endupload = -1;
   function uploadRecord(){
    var ss=getApplet().isFileExist();
    //alert(ss);
   if(ss==1){
   	 var upload=getApplet().uploadRecord();
     endupload = 0;
     $.unblockUI();
     clearInterval(uploadRecord_);
     }
...
function setStr(o){
		//alert(o);
  		var voiceAnswer = document.getElementById("voiceAnswer");
  		voiceAnswer.value=o;
  		//alert(voiceAnswer.value);
  		endupload=0;
  		endrec=0;
  		$("#voiceText").html(o);
}
...
//下一题
	...
	<!--
		...
		<table width="1147" height="610" border="0" align="center"
			cellpadding="0" cellspacing="0" style="margin-top:83px;"><!--sun 样式修改-->
			<tr>
				<td height="507">
					<table width="100%" border="0" cellspacing="0" cellpadding="0">
						<tr>
							<td width="693"  align="right" valign="top">
								<table width="560" border="0" cellpadding="0" cellspacing="0" style="margin-right:23px;">
									<tr>
											  ...
											  <tr>
											    ...
											    <td>
											    	<span class="STYLE1">得分：</span>
														...
														<span style="color:#EC0000;font-size:25px;font-weight:bold;" id="totalScore">
															<s:property value="myExamPaper.score" /></span>
									...
									<tr>
										<td width="535" height="460" valign="top">
											<div id="swfcontent" style="height:458px;width:650px;overflow:hidden">
                                ...
                                <tr>
                                	<td height="74" align="left" valign="middle">
                                    	<table border="0" cellpadding="0" cellspacing="0">
                                            	<tr>
                                                	<td width="60" align="left" valign="top"  style="padding-top:4px;">	
														
														<img height="45" width="45" src="images/20140702/help_l.png" 
														onmousedown="this.src='images/20140702/help_l.png'"
														onmouseup="this.src='images/20140702/help_l.png'"   style="cursor:pointer;"
														onclick="preview('http://10.1.1.40:8080/wjm/course/yanshi/juese/index.swf');"/>	<!--董克修改过<s:property value="helpSwf" />');-->																									</td>
                                                    ...
                                                    <td width="60" align="left" valign="top"  style="padding-top:4px;"><img height="45" src="images/20140702/fanhui.png"  width="45" id="fanhui" style="cursor:pointer;" /></td>
                                                    <td>
                                                    <script type="text/javascript">
	 ...
	 //$(timer).css("padding-top","8px");
	 $(timer).css("line-height","38px");
	 $(timer).css("border-width","0px");
	 $(timer).css("border-style","solid");
	 $(timer).css("border-color","green");
	 $(timer).css("background","url('images/datigaiban/daojishi.png')");
	 /**
	 var timer_h = $("<div>");
	 ...
	 */
	 ...
	 $(timer).append("<h2 style='margin:3px 0px 3px 0px;color:white;text-align:center;font-family:\"Times New Roman\",Georgia,Serif;' id='"+timerarea+"'>loading</h2>");
	 this.timerd = timer;
	 $("body").append(timer);
...
QuizTimer.prototype.start=function(){
	...
	$("#"+this.timeinput).attr("value", this.passtime+this.nowpassed);
									...
									<s:if test="view==0">
										<tr>
											<td  width="390" height="50" style="PADDING:15PX;">
												<span id="ques_stemText" style="color:white;font-size:18px;font-weight:bold;"></span>
												<!--
												<div>
									...
									<s:else>
										<tr>
											<td  width="390" height="50" style="PADDING:15PX;">
												<span id="ques_stemText" style="color:white;font-size:18px;font-weight:bold;">${question.stemText }</span>
											</td>
										</tr>
									</s:else>
									<!-- 语音识别文本 -->
									<tr>
										<td  width="390" height="50"  style="PADDING:15PX; font-size:18px;">
											<table width="100%" border="0" cellspacing="0" cellpadding="0">
  <tr>
    <td width="100" align="left" valign="top"><span style="color:black;font-size:18px;font-weight:bold;" id="beforeVoiceText"></span></td>
    <td valign="top" style="line-height:25px;"><span id="voiceText" style="color:white;font-size:18px;font-weight:bold;"></span></td>
  </tr>
</table>
									...
									<tr>
										...
										<table width="100%" border="0" cellspacing="0" cellpadding="0">
  <tr>
    <td width="100" align="left" valign="top" style="line-height:25px;"><span style="color:black;font-size:18px;font-weight:bold;" id="beforeYangyinText"></span></td>
    <td valign="top" style="line-height:25px;"><span id="ques_yangyinText" style="color:white;font-size:18px;font-weight:bold;"></span></td>
									...
									<tr>
										<td align="center">
											<form action="#" id="theform" method="post">
												...
												<s:hidden name="question.voiceAnswer" id="voiceAnswer"/>
												<s:hidden name="myExamPaper.id" />
												<input type="hidden" name="answerflag" id="answerflag"/>
							...
							<s:else>
								...
								<td width="65" align="center" valign="middle">
										<img src="images/bf/anniu_31.png"  id="pause_play"
											width="50" height="50" onclick="js_pause_play();" style="cursor:pointer;" />
							    </td>
							    <!-- 重置 -->
								<td width="65" align="center" valign="middle">
										<img src="images/datigaiban/reset.png" width="50" height="50"
		...
		<table border="0"  cellpadding="0" cellspacing="0" width="380">
            <tr>
                ...
                <td width="200px">
				<APPLET ID="录音功能(record)" CODE="com.msc.applet.RecognizeApplet.class"
						CODEBASE="/svn3/record" ARCHIVE="Msc.jar,json-lib.jar" WIDTH="80" HEIGHT="30" MAYSCRIPT>
						<PARAM NAME="type" VALUE="application/x-java-applet;version=1.2.2">
						<PARAM NAME="scriptable" VALUE="false">
						<PARAM NAME="filename" VALUE='<s:property value="#session.userId" />_<s:property value="myExamPaper.id" />_<s:property value="question.id" />.pcm'>
						<PARAM NAME="uploadUrl" VALUE="<%=basePath%>recordUpload">
					</APPLET>
                ...
                <td width="70">
                	<img src="record/img/anniu_speaker.png" 
						...
						width='50' height='50' 
						style="float:right; cursor:pointer;"  onclick="userspeak()"
						id="speaker" />
				</td>
                <td width="50">
