	...
	function checkInit(){
		if(init == 1){
		   ...
		   //播放视频
			...
			if(view == 0){
				//点击备选项的操作
				$("#bx_answer table").click(function() {
					var jValue = $(this).find("td").eq(1).text();
					//生成需要追加的html
					var jHTML = "<table  border='0' cellspacing='0' cellpadding='0'  >"+
							"<tr>"+
								"<td width='16'>"+
									"<img src='images/px_images/N11_0101.png' width='16'"+
										"height='35' />"+
								"</td>"+
								"<td style='background:url(images/px_images/N11_0202.png) repeat-x center center;'>"+
									"<div class='bx_answeri' style='color:#000;'>"+
										jValue+
									"</div>"+
								"</td>"+
								"<td width='16'>"+
									"<img src='images/px_images/N11_0303.png' width='16'"+
										"height='35' />"+
								"</td>"+
							"</tr>"+
						"</table>";
					var at = $("<table>");
					$(at).attr("border","0");
					$(at).attr("cellspacing","0"); 
					$(at).attr("cellpadding","0");
					//$(at).attr("style","margin:5px");
					$(at).append($(jHTML).html());
					var ai = $(at).find(".bx_answeri")[0];
					var p = $(ai).parent();
					$(at).attr("id","an_"+$.trim($(ai).text()));
					
					if($("#an_"+$.trim($(ai).text())).length==0){
						$("<input type=\"hidden\" name=\"question.stuAnswers\" value=\""
							+$.trim($(ai).text())+"\"/>").appendTo($(p));
						$("#s_answer").append($(at));
					}
				});
			}
...
$(function(){
		...
		var top = $(document.body).height()/2 + 150;
		$("#youxiajiao").css({"position":"absolute","top":top,"left":left});
		init_statu = setTimeout("checkInit()",1000);
		
});
//点击已选项的操作,使用代理
$("#s_answer>table").live("click",function(){
	var table = $(this).parent();
	var ai = $(this);
	var p = $(ai).parent();
	if($("#an_"+$.trim($(ai).text())).length==1){
		$("#an_"+$.trim($(ai).text())).remove();
	}
});
...
//20141106添加一次答对判断
if(examType==1){
	if($("#next").attr("src")=='images/datigaiban/anniu_32.gif'){
		jjAlert("您的答题次数以到，请继续作答");
		return false;
	}
}
...
#apDiv1 {
	...
	<body onload="load();" style="min-height:650px;" ><!--sun 样式修改-->
		...
		<!-- 答对提示音 -->
		<div style="display: none;" id = "objects">
			...
			<s:if test="view == 1">
				...
				<object id=recoding
					...
					Microsoft? Windows Media? Player components...
					type=application/x-oleobject>
					<param name=URL value="<s:property value="question.recoding" />">
					<PARAM NAME=UIMode value=full>
					<PARAM NAME=AutoStart value=false>
					<PARAM NAME=Enabled value=true>
					<PARAM NAME=enableContextMenu value=false>
					...
					<table width="100%" border="0" cellspacing="0" cellpadding="0">
						<tr>
							<td width="693" height="507" align="right" valign="top">
								<table width="650" border="0" align="right" cellpadding="0"
									cellspacing="0" style="margin-right: 23px">
									<tr>
  ...
  <tr>
    ...
    <td><span class="STYLE1">得分：</span>
											<span
												style="color:#EC0000;font-size:25px;font-weight:bold;" id="totalScore"><s:property
														value="myExamPaper.score" />
							...
							<td valign="top" style="padding-left:0px;padding-right:20px;">
							  <table width="100%" border="0" align="right" cellpadding="0" cellspacing="0">
                                        	...
                                        	<table border="0" cellspacing="0" style="margin-left:0px;margin-top:3px;">
                                            	<tr>
                                                	<td width="60" align="left" valign="top" style="padding-top:4px;"><img height="45" width="45" src="images/20140702/help_l.png" 
														...
														<!--董克修改过preview('<s:property value="helpSwf" />')-->												</td>
									...
									<tr>
										<td  height="114" valign="top" style="padding-top:10px;">
											<div class="bx_answer" id="bx_answer" >
												<s:iterator value="question.answers_">
													<table  border="0" cellspacing="0"
														cellpadding="0">
														<tr>
															<td width="16">
																<img src="images/px_images/N11_01.png" width="16"
																	height="35" />
															</td>
															<td style="background:url(images/px_images/N11_02.png) repeat-x center center;">
																<div class="bx_answeri">
																	<s:property escape="false" />
																</div>
															</td>
															<td width="16">
																<img src="images/px_images/N11_03.png" width="16"
																	height="35" />
															</td>
														</tr>
													</table>
									...
									<s:if test="view==0">
										<tr>
											<td  height="114" valign="top" style="padding-right:10px;">
												<div>
													<form action="#" id="theform" method="post">
														<div class="s_answer" id="s_answer">
										...
										<tr>
											<td valign="top" >
												<div style="padding-right:15px;font-size:18px;">
  ...
  <tr>
    ...
    <td valign="top" style="line-height:25px;"><span id="ques_yangyinText" style="color:white;font-size:18px;font-weight:bold;"></span></td>
		...
		<table cellpadding="0" cellspacing="0" width="280px">
             <tr>
                 <td width="50px">
                 	<div id="r_o_w_img" style="width:50px;height:50px;"></div>
                 </td>
                 <td width="100px"><img name="soundwave" id="soundwave"  width='120' height='44' src="record/img/mic_t.png" 
                 			onclick="recordFunc();" style=" float:right;" />
         </td>
                 <td width="70"><img src="record/img/anniu_speaker.png" 
							onmousedown="this.src='record/img/ann_speaker.png'" 
							onmouseup="this.src='record/img/anniu_speaker.png'" width='50' height='50' 
							onclick="userspeak();" style=" display:none; float:right; cursor:pointer;" id="speaker"/>
				 </td>
                 <td width="50"><img src="images/datigaiban/tj.png"  
                            onmousedown="this.src='images/datigaiban/tj_h.png'"
							onmouseup="this.src='images/datigaiban/tj.png'" align="right" id="tj" width="50" height="50"
