...
public class IndexSystemConfigOp {
	
	private static final Log logger = LogFactory.getLog(IndexSystemConfigOp.class);
	private static String path = "";
	private static Map<String, String> indexSystemconfig;
	
	public static String getValue(String key){
		if(!indexSystemconfig.containsKey(key)) return "无记录";
		return indexSystemconfig.get(key);
	}
	public static int getIntValue(String key){
		if(!indexSystemconfig.containsKey(key)) return 0 ;
		return new Integer(indexSystemconfig.get(key));
	}
	public static boolean getBooleanValue(String key){
		if(!indexSystemconfig.containsKey(key)) return false ;
		return  indexSystemconfig.get(key).equals("true")? true:false;
	}
	public static void setPath(String path) {
		IndexSystemConfigOp.path = path;
	}
	@SuppressWarnings("unchecked")
	public static void load() throws Exception{
		Properties ps = new Properties();
		FileInputStream fis=new FileInputStream(path+"/indexSystemconf.properties");
		ps.load(fis);
		fis.close();
		Enumeration enum1 = ps.propertyNames();
		indexSystemconfig = new HashMap<String, String>();
		while (enum1.hasMoreElements()) {
			String key = (String) enum1.nextElement();
			String value = ps.getProperty(key);
			indexSystemconfig.put(key, value);
		}
	}
	public static void setProperty(String id,int value)throws ElException{
		try {
			...
			fis.close();
			per.put(id, value+"");
			FileOutputStream fos=new FileOutputStream(path+"/indexSystemconf.properties");
			per.store(fos, "");
			fos.close();
			load();
		} catch (Exception e) {
	...
	public static void setProperty(String id,boolean value)throws ElException{
		try {
			...
			logger.error ("设置系统首页配置失败",e);
			throw  new ElException( "设置系统首页配置失败",e);
		}
	}
