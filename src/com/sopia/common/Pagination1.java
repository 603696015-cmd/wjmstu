 ...
 *
 */
public class Pagination1 extends TagSupport {
	private static final long serialVersionUID = 3119679319963664116L;
	private static final Log logger = LogFactory.getLog(Pagination1.class
			);
	...
	public int doStartTag() {
		try {
			...
			if (pageNow > 0) {
				out.print("<a style='cursor: hand' href='javascript:page1("
						+ (0) + ")'>[首页]</a>");
				out.print("<a style='cursor: hand' href='javascript:page1("
						+ (pageNow - 1) + ")'>[上一页]</a>");
			} else {
			...
			if (pageCount > 0) {
				out
						.print("<select  onchange='page1(this.options[this.selectedIndex].value)'>");
