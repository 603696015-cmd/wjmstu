...
plan.list.byuid = select * from (select p.id,p.name,p.content,p.manager,p.contact,p.participator,eu.realname,p.createtime,p.status,row_number() over(order by p.createtime desc) rownum from elplan p,eluser eu where p.manager = eu.id and  p.manager = ? )t where t.rownum between ? and ?
...
plan.verify.list = select * from (select p.id,p.name,p.manager,p.createtime,p.status,eu.realname,row_number() over(order by eplv.verifydate desc ) rownum from elplanverify eplv,eluser veu,department vdep,elplan p,eluser eu   where p.id = eplv.planid and p.manager = eu.id and eplv.status=2 and veu.id = eplv.userid and veu.depid = vdep.id and vdep.parentid=? and eplv.superverify=0)t where t.rownum between ? and ?
...
plan.carryout.list =select * from (select p.id,p.name,p.manager,eu.realname,p.createtime,p.status, \
					  sum(elps.plandays)  as pldays, \
					  sum(elps.realdays) as redays, \
					  p.finishdate, max(realfinishdate) as maxf, row_number() over (order by p.createtime desc ) rownum \
					from elplan p left join eluser eu on eu.id = p.manager \
					left join department dep on dep.id = eu.depid and dep.lid>=? and dep.rid <= ? \
					left join elplanstage elps on elps.planid = p.id \
					where (p.status = 2 or p.status=4)  group by p.id,p.name,p.manager,eu.realname,p.createtime,p.status, p.finishdate )t where t.rownum between ? and	?
...
survey.mylist=select* from(select  s.id, s.title,s.begintime,s.endtime,s.epid,ep.title eptitle,s.stureadresult,row_number() over(order by s.begintime desc) rownum from elsurvey s,exampaper ep where s.epid = ep.id and s.creater = ? )t where t.rownum between ? and ?
...
survey.list.bydepid =select * from ( select es.id,es.title,es.creater,eu.realname,es.begintime,es.endtime,es.epid,ep.title eptitle,dep.id depid,dep.name depname,row_number() over(order by es.begintime desc) rownum from elsurvey es ,eluser eu,exampaper ep,department dep \
                where es.creater = eu.id and eu.depid = dep.id and es.epid = ep.id and dep.lid<=? and dep.rid>=? )t where t.rownum between ? and ?
...
poll.mylist=select * from (select s.id, s.title,s.begintime,s.endtime,s.qid,q.title qtitle ,s.stureadresult,row_number() over( order by s.begintime desc)  rownum from elpoll s,question q where s.qid = q.id and s.creater = ?  )t where t.rownum between ? and ?
poll.mylist.size=select count(*) from elpoll s,question q where s.qid = q.id and s.creater = ?
poll.list.bydepid = select * from (select es.id,es.title,es.creater,eu.realname,es.begintime,es.endtime,es.qid,ep.title eptitle,dep.id depid,dep.name depname ,row_number() over(  order by es.begintime desc)  rownum  from elpoll es ,eluser eu,question ep,department dep \
                where es.creater = eu.id and eu.depid = dep.id and es.qid = ep.id and dep.lid<=? and dep.rid>=? )t where t.rownum between ? and ?
...
offline.list = select * from (select o.id, o.name,o.description,o.during,o.xueshi,o.score,o.begintime,o.endtime,count(ou.userid) oucount,row_number() over(order by o.begintime desc) rownum from eloffline o left join eloffline2user ou on ou.offid = o.id group by  o.id, o.name,o.description,o.during,o.xueshi,o.score,o.begintime,o.endtime ) t where t.rownum between ? and ?
