...
public class PG_PolicyAction extends BaseAction {
	...
	public String policyLib_addInit() throws ElException {
		if (getSessionIntValue(ElConstants.SESSION_ROLE) == 1) {
			policyLib = PG_policyDao.getPolicyLibTree(ElConstants.TREE_ROOT,
					ElConstants.TREE_FIANL, true);
		} else {
			policyLib = PG_policyDao.getPolicyLibTree(
	...
	public String policyLib_add() throws ElException{
		PG_policyDao.addPolicyLib(ptype); 
	...
	public String policyLibListInit() throws ElException{
		policyLib = PG_policyDao.getPolicyLibTree(ElConstants.TREE_ROOT,ElConstants.TREE_FIANL, true);
		return "policyLibList";
	}
	public String policyLib_view() throws ElException{
		policyLib = PG_policyDao.getPolicyLibTree(ElConstants.TREE_ROOT,ElConstants.TREE_FIANL, true);
		ptype = PG_policyDao.getPtypeByid(ptype.getId()); 
		ptype.setOpusers(PG_policyDao.getOpUsers( ptype.getId()));
	...
	public String policyLib_alter() throws ElException {
		...
		PG_policyDao.alterpolicylib(ptype);
	...
	public String policyLib_delete() throws ElException{
		...
		if(productIsDel == 1){
			//并入上级	3	1
			PG_policyDao.updatePolicyTypeParentid(ptype.getId(), ptype.getParent().getId());
			PG_policyDao.updatePolicyParentid(ptype.getId(), ptype.getParent().getId());
			PG_policyDao.deletePtype(ptype.getId());
		}else{
			//一起删除
			PG_policyDao.deletePolicylibAndSub(ptype.getId());
		}
		//更新树左右id
		((ElNodeSQL)SpringContextUtil.getBean("elnodesql")).updatetlrid("policylib");
		//刷新首页新闻模块
...
//--------------下面是保单信息 
	...
	public String MyPolicyListInit() throws ElException{ 
		...
		policys = PG_policyDao.getPolicyList(policy,nid, ptypeTree,true,getPageNow(), getPageSize());
		count = PG_policyDao.getPolicyListSize(policy,nid, ptypeTree,true);
		return "MyPolicyList";
	} 
	...
	public String Policy_AuditListInit() throws ElException{ 
		...
		policy = new Policy();
		policy.setCreateId(new ELUser(getSessionIntValue(ElConstants.SESSION_USERID)));  
		policys = PG_policyDao.getPolicyList(policy,nid, ptypeTree,false,getPageNow(), getPageSize());
		count = PG_policyDao.getPolicyListSize(policy,nid, ptypeTree,false);
	...
	public String Policy_AuditInit() throws ElException{ 
		...
		if(policy !=null)
			PG_policyDao.alterPolicyValid(policy.getId(), policy.getValid());
	...
	public String Policy_UpdateScanning() throws ElException, Exception {
		...
		if (st.length() > 10 * 1024 * 1024) {
			...
			PG_policyDao.updateScanning(policy.getId(), ".jpg"); 
	...
	public String buyPolicyView()throws ElException{
		baoxianProduct = PG_baoxianProductDao.showBaoxianProduct(baoxianProduct.getId());
