...
public class BtypeList extends ElTag{
	private static final long serialVersionUID = 3119679319963664116L;
	private static final Log logger = LogFactory.getLog(BtypeList.class);
	...
	public int doStartTag() {
		try {
			...
			BookType qlb = (BookType) request.getAttribute("bookTree");
			writeChilds(out,qlb ) ;
		} catch (Exception ex) {
			logger.error("图书类别列表错误",ex);
		}
