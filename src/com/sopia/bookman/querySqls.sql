...
book.query.byuid=select n.id,n.title,n.releasetime,n.btid ,nt.name,n.pubhouse,n.writer,n.pubtime  from book n,booktype nt where nt.id = n.btid and nt.lid>=? and nt.rid <=? and n.userid =? order by n.releasetime desc limit ?,?
...
book.dep.list=select n.id,n.title,n.releasetime,n.btid ,nt.name,n.pubhouse,n.writer,n.pubtime,eu.id,eu.realname,hot from book n,booktype nt,eluser eu,department dep where nt.id = n.btid and eu.id = n.userid and dep.id = eu.depid and dep.lid>=? and dep.rid <=? order by n.releasetime desc limit ?,? 
