...
study.course.query.byuid=select * from (select c.id cid, c.name,c.creater, \
							   	...
							   	row_number() over (order by c.createtime) rownum \
							   	from study_course sc left join course c on sc.courseid = c.id \
							   	left join eluser eu on c.creater = eu.id left join study_quizinfo sqi on sqi.id=sc.sqiid where sc.userid = ? and sc.status=?) t where t.rownum between ? and ?
study.course.query.size.byuid=select count(*) from study_course sc where sc.userid = ? and sc.status=?
study.course.querywithoutstatus.byuid=select * from (select c.id cid, c.name,c.creater, \
							   	eu.realname,c.credit, \
							   	c.during,c.teachername,sc.passtime/60 passtime,sc.process,sc.status,c.islink,sc.mycredit,sqi.id _sqiid,sqi.myScore,sqi.ispassed, \
							   	row_number() over (order by sc.status asc, c.createtime desc) rownum \
							   	from study_course sc left join course c on sc.courseid = c.id \
							   	left join eluser eu on c.creater = eu.id left join study_quizinfo sqi on sqi.id=sc.sqiid where sc.userid = ?) t where t.rownum between ? and ?
...
study.course.canapply.from.this=select * from (select c.id,c.name,c.ctypeid, c.status,c.createtime,c.modifytime,c.creater,ct.name ctmane,u.realname,c.credit,row_number() over (order by c.createtime desc) rownum from course c, COURSE_TYPE ct,ELUSER u,DEPARTMENT dep \
 		where c.ctypeid=ct.id and c.creater = u.id and u.depid=dep.id and c.status = ? and c.name like ? and dep.id=?  and ct.lid>=? and ct.rid<=?) t where t.rownum between ? and ?
study.course.canapply.from.super=select * from (select c.id,c.name,c.ctypeid, c.status,c.createtime,c.modifytime,c.creater,ct.name ctmane,u.realname,c.credit,row_number() over (order by c.createtime desc) rownum from course c, COURSE_TYPE ct,ELUSER u,course_dep cd \
		where c.ctypeid=ct.id and c.creater = u.id and cd.courseid = c.id and c.status = ? and c.name like ? and cd.depid=? and c.id  and c.ctypeid=?) t where t.rownum between ? and ?
...
study.course.ph=select * from(select c.id,c.name,c.ctypeid,ct.name ctname,c.creater,eu.realname,c.createtime, \
					  count(ca.courseid )as cstu ,row_number() over (order by count(ca.courseid ) desc) rownum \
					from course c \
					...
					group by c.id,c.name,c.ctypeid,ct.name,c.creater,eu.realname,c.createtime )t where t.rownum between ? and ?
...
study.class.byuid = select cl.id,cl.name,cl.optionalcredit,cl.createtime,ca.applyDate ,\
					...
					eu.id,eu.realname,cl.certificatename  from study_class ca ,elclass cl,eluser eu where cl.creater = eu.id and ca.userid = ? and ca.classid = cl.id and ca.status = 2
...
study.class.dep.passper=select * from (select dep.id,dep.name, count(eu .id) stucount  , count(ca.userid) passcount ,row_number() over(order by cast( count(ca.userid)as  decimal)/cast( count(eu .id)as  decimal) desc) rownum from department dep \
					...
					and eu.id not in (select userid from elgroup2user where gid = ?) where dep.parentid = 1  and dep.id not in(420 ,419)  group by dep.id,dep.name ) t where t.rownum between ? and ?  
######study .cpage###########
...
study.cpage.lastid =select top 1 sc.cpid from study_cpage sc,course_page cp where sc.cpid= cp.id  and sc.userid = ? and cp.courseid= ? order by sc.endtime desc 
...
study.qpaper.finalscore.set=update study_quizinfo set myScore = ?,status = 3,ispassed= ?,score=? where id = ?
...
study.qpaper.recent.list= select * from ( select sqi.id sqiid, er.id erid,er.title,er.location , er.begintime,er.endtime,row_number() over ( order by er.begintime desc) rownum from exam_room er left join study_quizinfo sqi on er.id = sqi.roomid where sqi.userid = ?)t where t.rownum  between ? and ?
...
study.qpaper.list= select * from ( select er.id erid,er.title,er.location, er.begintime,er.endtime erendtime,c.id cid,c.name,c.status cstatus,sqi.id sqid,sqi.myscore,sqi.status sqistatus,sqi.endtime sqiendtime,row_number() over(order by er.begintime desc ) rownum from (select * from exam_room where iscommon = 0) er left join study_quizinfo sqi on er.id = sqi.roomid left join course c on c.id = er.courseid left join study_course ca on ca.courseid=c.id and sqi.userid=ca.userid where sqi.userid = ? )t  where t.rownum between ? and ?
...
study.qpaper.withoutcourse=select * from (select er.id erid,er.title,er.location ,er.begintime,er.endtime,sqi.id sqiid,sqi.status,row_number() over ( order by er.begintime desc) rownum from (select * from exam_room  where iscommon=1) er   left join study_quizinfo sqi on er.id = sqi.roomid where sqi.userid =? ) t where t.rownum between ? and ?
...
study.qpaper.withoutcourse.result=select * from (select sqi.id ,sqi.userid, sqi.roomid, sqi.epid, sqi.status, sqi.myScore,\
						sqi.endtime,er.title,sqi.begintime,sqi.score sqiscore,er.score erscore,row_number() over(order by er.begintime) rownum from  study_quizinfo sqi left join EXAM_ROOM er on \
						sqi.roomid = er.id where sqi.userid = ? and sqi.status=3 and  er.iscommon=1)t where t.rownum between ? and ? 
study.mroom.withoutcourse=select * from (select er.id erid,er.title,er.location ,er.begintime,er.endtime,sr.status,count(sqi.id) sqicount,row_number() over ( order by er.begintime desc) rownum from (select * from exam_room  where iscommon=1) er left join study_room sr on er.id = sr.roomid left join (select * from study_quizinfo where userid = ?) sqi on sqi.roomid =sr.roomid where  sr.userid =? group by er.id,er.title,er.location ,er.begintime,er.endtime,sr.status ) t where t.rownum between ? and ?
...
study.myexamprac.list=select * from (select epr.id,epr.title,epr.begintime,epr.endtime,epr.epid, row_number() over(order by id ) rownum from examprac epr left join examprac_assign epra on epra.eprid= epr.id where epra.userid =?   )t  where t.rownum between ? and ?					


study.canapplyroom.list=select * from (select er.id , er.title, er.begintime, er.endtime,er.location,er.passgrade,er.erlibid,erlib.name,er.type ,count(reps.epid) epsize ,row_number() over( order by er.begintime desc)rownum from exam_room er left join eroom_lib erlib on erlib.id=er.erlibid left join exam_reps reps on reps.roomid = er.id where er.iscommon=1 and er.type=2 group by  er.id , er.title, er.begintime, er.endtime,er.location,er.passgrade,er.erlibid,erlib.name,er.type)t where t.rownum between ? and ?
