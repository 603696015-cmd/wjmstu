...
talent.trcoll.list=select * from (select id,title,description,createtime,row_number() over(order by createtime desc ) rownum from troomcoll where creater = ? )t where t.rownum between  ? and ?
...
talent.troom.assigned.user.list= select *from (select tra.userid,eu.realname,eu.depid,dep.name,eu.username ,row_number() over(order by tra.assigntime desc ) rownum from troom_assign tra,eluser eu,department dep where eu.depid=dep.id and eu.id = tra.userid and tra.trid =? )t where t.rownum between ? and ? 
...
##########student####
talent.troom.list.bystuid =select * from ( select tr.id,tr.title,tr.description,tr.createtime,tr.creater,eu.realname, sum(ti.myscore) myscore,count(str.id) strc,row_number() over(order by tr.createtime desc )rownum from \
			troom_assign tra \
			left join troomcoll tr on tra.trid = tr.id \
			left join eluser eu on eu.id=tr.creater \
			left join troom_epinfo ti on  ti.userid = tra.userid \
			left join troom str on str.id = ti.trid and str.trcid = tr.id \
			where tra.userid = ? group by \
			tr.id,tr.title,tr.description,tr.createtime,tr.creater,eu.realname ) t where t.rownum between ? and ? 
...
talent.ztroom.list.byuid = select * from (select id,title,description,begintime,endtime,norm,row_number() over(order by begintime desc ) rownum from ztroom where creater = ?) t where t.rownum between ? and ?
...
talent.ztroom.assigned.zuser.list= select *from(select tra.userid,eu.realname,eu.depid,dep.name,eu.username,row_number() over( order by tra.assigntime desc) from ztroom_assign tra,eluser eu,department dep where eu.depid=dep.id and eu.id = tra.userid and tra.trid =?) t where t.rownum between ? and ? 
...
talent.myztroom.list.bystid = select * from (select ztr.id,ztr.title,ztr.description,ztr.begintime,ztr.endtime,ztr.norm,ztr.creater,eu.realname,ztra.zjscore,ztra.tsscore,ztra.sjscore,row_number() over(order by ztr.begintime desc) rownum  from ztroom ztr ,ztroom_assign ztra,eluser eu where ztr.creater = eu.id and ztra.trid = ztr.id and  ztra.userid = ? ) t where t.rownum between ? and ?
