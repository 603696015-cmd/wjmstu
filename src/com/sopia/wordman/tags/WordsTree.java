package com.sopia.wordman.tags;
...
public class WordsTree extends ElTag {
	private static final long serialVersionUID = 3119679319963664116L;
	private static final Log logger = LogFactory.getLog(WordsTree.class);
	@SuppressWarnings("unchecked")
	public int doStartTag() {
		try {
			JspWriter out = pageContext.getOut();
			ServletRequest request = pageContext.getRequest();
			Word ws = (Word) request.getAttribute("wordsTree");
			...
			out.println("var d"
					...
					+ getIname()
					+ "',"
					+ (getIvalue() == null || "".equals(getIvalue()) ? ws.getId()
							: getIvalue()) + ", 'd" + getDid() + "');\n");
			if(getItype()!= null && getItype().equals("OP")){//OP  可操作类型
				writeChildsOP(out, ws , role);
			}else if(getItype()!= null && getItype().equals("cb_2")){
				writeChildsCb_2(out, ws);
			}else{
				writeChilds(out, ws);				
			}
	...
	public void writeChilds(JspWriter out, Object obj) throws Exception {
		Word qlb = (Word) obj;
		List<Word> qlbChild = qlb.getChild();
