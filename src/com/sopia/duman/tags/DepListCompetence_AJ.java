...
public class DepListCompetence_AJ extends ElTag {
	private static final long serialVersionUID = 3119679319963664116L;
	private static final Log logger = LogFactory.getLog(DepList_AJ.class);
	...
	public int doStartTag() {
		try {
			...
			Kledge kledge = (Kledge)request.getAttribute("kledge");
			Department cts = null;
			
			if(this.getCompetenceType() == 1){
				cts = kledge.getDep_view();
			}else if(this.getCompetenceType() == 2){
				cts = kledge.getDep_update();
			}else if(this.getCompetenceType() == 3){
				cts = kledge.getDep_delete();
			}else if(this.getCompetenceType() == 4){
				cts = kledge.getDep_copy();
			}else if(this.getCompetenceType() == 5){
				cts = kledge.getDep_download();
			}
