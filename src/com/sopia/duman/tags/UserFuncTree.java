...
public class UserFuncTree extends ElTag{
	/**
	 * 
	 */
	private static final long serialVersionUID = 5714530967503948064L;
	private static final Log logger = LogFactory.getLog(UserFuncTree.class);
	private int userid;
...
//			HttpServletRequest req = (HttpServletRequest) request;
			userid=0;
			if (null != request.getAttribute("elUser")) {
				ELUser eu = (ELUser ) request.getAttribute("elUser");
				userid= eu.getId();
			}
...
//		}
		...
		String desc=qlb.getName()+"-"+qlb.getDescription()+"==="+AuthorityUtil.checkUserfuncAuthor(userid, qlb.getId());
		...
		if(n==0){
			...
			//out.println("d" + getDid() + ".add("+(getDid()-9)+",-10,'"+qlb.getName() +"模块功能');");
			n=1;
		}
		//out.println("d" + getDid() + ".add("+qlb.getId()+","+qlb.getParent().getId()+",'"+desc+"','"+href1+"');\n");
...
//			out.println("--");
//		}
		String desc=qlb.getName()+"-"+qlb.getDescription()+"==="+AuthorityUtil.checkAuthor(userid, qlb.getFunccode(),0);
		//String desc=qlb.getName();
		desc=desc.replace("\n", "");
		desc=desc.replace("\r", "");
		//if(qlb.getLevel()==0){
