...
user.query.byid=select eu.id,eu.username,eu.password,eu.realname,eu.userno,eu.phone,eu.address,eu.email,eu.role,eu.depid,dep.name,eu.valid,er.name, eu.sex, eu.age , eu.edubg , eu.major , eu.studyDir , eu.gradchool , eu.graddate , eu.jobdate , eu.protitle , eu.jobdesc ,eu.majorc,eu.score,eu.dot,eu.xfscore,eu.valid \
			from ELUSER eu left join DEPARTMENT dep on eu.depid = dep.id left join elrole er on er.id = eu.role where eu.id=?
user.query.un=select eu.id,eu.username, eu.password,eu.realname,eu.userno, eu.phone,eu.address,eu.email, eu.role,eu.depid,eu.valid,er.name,eu.station from eluser eu,elrole er where er.id = eu.role and  eu.username=?
user.query.bydepid=select eu.id,eu.username from ELUSER eu where eu.depid =?
user.query.subs.bydepidandos=select * from (select eu.id,eu.username, eu.realname,eu.userno,eu.email,eu.role,dep.id depid ,dep.name depname ,eu.valid,er.name ername ,row_number() over(order by eu.id asc) rownum from ELUSER eu left join elrole er on er.id = eu.role left join DEPARTMENT dep on eu.depid = dep.id \
			where eu.username like ? and eu.realname like ? and eu.email like ? and dep.lid >=? and dep.rid<=? ) t where t.rownum between ? and ?
...
user.query.bydepidandos=select * from (select eu.id,eu.username, eu.realname,eu.userno,eu.email,eu.role,dep.id depid,dep.name depname,eu.valid,er.name ername, row_number() over(order by eu.id asc) rownum  from ELUSER eu left join DEPARTMENT dep on eu.depid = dep.id left join elrole er on er.id = eu.role \
			where eu.username like ? and eu.realname like ? and eu.email like ? and dep.id=? ) t where t.rownum between ? and ?
...
user.query.vsubs.bydepidandos=select * from (select eu.id,eu.username, eu.realname,eu.userno,eu.email,eu.role,dep.id,dep depid.name depname,eu.valid,row_number() over(order by eu.id asc) rownum from ELUSER eu left join DEPARTMENT dep on eu.depid = dep.id \
...
user.query.vbydepidandos=select * from (select eu.id,eu.username, eu.realname,eu.userno,eu.email,eu.role,dep.id depid,dep.name depname,eu.valid,row_number() over(order by eu.id asc) rownum from ELUSER eu left join  DEPARTMENT dep on eu.depid = dep.id \
...
user.query.subs.bydepidandos.role=select * from (select eu.id,eu.username, eu.realname,eu.userno,eu.email,eu.role,dep.id depid,dep.name depname,eu.valid,er.name ername,row_number() over(order by eu.id asc) rownum from ELUSER eu left join elrole er on er.id = eu.role left join DEPARTMENT dep on eu.depid = dep.id \
			where eu.username like ? and eu.realname like ? and eu.email like ? and dep.lid >=? and dep.rid<=? and eu.role=?  ) t where t.rownum between ? and ?
...
user.query.bydepidandos.role=select * from (select eu.id,eu.username, eu.realname,eu.userno,eu.email,eu.role,dep.id depid,dep.name depname,eu.valid,er.name ername,row_number() over(order by eu.id asc) rownum  from ELUSER eu left join DEPARTMENT dep on eu.depid = dep.id left join elrole er on er.id = eu.role \
			where eu.username like ? and eu.realname like ? and eu.email like ? and dep.id=? and eu.role=?   ) t where t.rownum between ? and ?
...
user.group.assign.list=select * from (select eu.id,eu.realname,dep.id depid,dep.name,eu.username,row_number() over(order by eu.id asc ) rownum from elgroup2user gu,eluser eu,department dep where dep.id=eu.depid \
							and eu.id=gu.userid and gu.gid = ? ) t where t.rownum between ? and ?
...
user.func.unclist = select id,funccode from elfunc where needcheck = 0
