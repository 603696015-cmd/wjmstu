...
public class PolicyDaoImpl extends ElNodeDao implements PolicyDao { 
	private static final Log logger = LogFactory.getLog(PolicyDaoImpl.class);
	...
	public int addPolicy(Policy policy) throws ElException{
		...
		int id = 0 ;
		try {  
			sql = "insert into policy (LIBID,COMMODITYID,IC_TABLENAME,IC_U_ID,SUBMITTIME,createid) values (?,?,?,?,?,?)";
			ct = DBConnection.getConnection();
			ps = ct.prepareStatement(sql);  
	...
	public void alterPolicyValid(int policyId , int valid) throws ElException {
		...
		try {
			ct = DBConnection.getConnection();
			sql = "update policy set valid=? where id=?";
			ps = ct.prepareStatement(sql);
			ps.setInt(1, valid);
	...
	public Policy getPolicyById(int id) throws ElException{ 
		...
		Policy p = new Policy();
		try { 
			sql = " select p.id,pb.name,p.valid,p.submittime,p.starttime ,p.IC_tablename,p.Ic_u_Id,p.commodityId from policy p left join" +
					" eluser eu on p.createid = eu.id left join  baoxian_product_lanmu bpl on p.libid = bpl.id left join " +
