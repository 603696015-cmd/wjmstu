...
public class BaoxianProductDaoImpl implements BaoxianProductDao{
	private static final Log logger = LogFactory.getLog(BaoxianProductDaoImpl.class);
...
//		return pt;
	...
	public void deleteBaoxianProduct(int id) throws ElException {
		...
		try {
			...
			ps = ct.prepareStatement(
					"delete from product_baoxian where id=?"
					);
			ps.setInt(1, id);
			ps.executeUpdate();
		} catch (Exception e) {
	...
	public int getRoleId(int id) throws ElException {
		...
		try {
			ct = DBConnection.getConnection();
			String sql = "select e.role from product_baoxian p join eluser e on p.userid=e.id where e.id=?";
	...
	public BaoxianProduct showBaoxianProduct(int id) throws ElException {
		...
		try {
			ct = DBConnection.getConnection();
			sql = "select p.*,pl.lanmu,e.username,i.id as i_id,i.name as i_name from product_baoxian p,baoxian_product_lanmu pl,eluser e,ic_manage i where p.suoshulanmu=pl.id and p.userid=e.id and p.insuranceCategoryId=i.id and p.id=?";
			ps = ct.prepareStatement(sql);
			ps.setInt(1, id);
			rs = ps.executeQuery();
			while (rs.next()) {
	...
	public void updateBaoxianProduct(int roleId, BaoxianProduct baoxianProduct,int shenhezhuangtai,boolean is_product_fabu_can_alter)
		...
		try {
			...
			ps.executeUpdate();
			OracleBlob setblob = new OracleBlob("product_baoxian","id",baoxianProduct.getId()+"","jianjie",baoxianProduct.getJianjie(),"修改计划失败",ct);
	...
	public void addBaoxianProduct(boolean is_baoxian_product_sh,BaoxianProduct baoxianProduct,ELUser elUser,int ptype_parent_id)
		...
		ResultSet rs = null;
		Connection ct = null;
		String sql = "insert into product_baoxian (id,name,jieshao,suoshulanmu,key," +
		...
		" values (product_baoxian_sequence.nextval,?,?,?,?," +
		"?,?,?,?," +
		"?,?,?,?,sysdate,?,empty_blob(),?,?,?,?,?,?,?)";
		try {
			ct = DBConnection.getConnection();
			...
			OracleBlob setblob = new OracleBlob(ct,"product_baoxian_sequence","product_baoxian","id","jianjie",baoxianProduct.getJianjie(),"添加保险产品失败");
			setblob.addContent();
	...
	public List<BaoxianProduct> getAllBaoxianProductByUidByPerOrShar(int nid,
		...
		try {
			...
			sql = 
				...
				"p.fabuzhe,p.fabuzhesuozaidanwei,p.fabushijian,p.suoshulanmu,p.zhengzhantuijian ,pl.lanmu,ps.shenhezhuangtai as psshenhezhuangtai  from product_baoxian p,baoxian_product_lanmu pl,product_shenhezhuangtai ps " +
	...
	public int getAllBaoxianProductCountByUidByPerOrShar(int nid,
		...
		try {
			...
			ct = DBConnection.getConnection();
			sql = "select count(*) from product_baoxian p, " +
...
//			Typelrid typelrid=this.getLidRid(ct,id, "newstype");
	...
	public void deleteProductByTypeid(Connection ct,int typeid) throws ElException {
		...
		ResultSet rs = null;
		try {
			ps = ct.prepareStatement("delete from product_baoxian where suoshulanmu=?");
			ps.setInt(1, typeid);
			ps.executeUpdate();
		} catch (Exception e) {
	...
	public void updateProductParentid(int pid, int npid) throws ElException {
		...
		try {
			ct = DBConnection.getConnection();
			ps = ct.prepareStatement("update product_baoxian set suoshulanmu=? where suoshulanmu=? ");
			ps.setInt(1, npid);
			ps.setInt(2, pid);
			ps.executeUpdate();
		} catch (Exception e) {
	...
	public BaoxianProduct getBaoxianProductByid(int id) throws ElException {
		...
		try {
			ct = DBConnection.getConnection();
			sql = "select p.*,ic.name as icname,ic.id as icid,ic.tablename as ictablename  from product_baoxian p join ic_manage ic on p.insurancecategoryid=ic.id where p.id=?";
			ps = ct.prepareStatement(sql);
			ps.setInt(1, id);
			rs = ps.executeQuery();
			if (rs.next()) {
	...
	public InsuranceCategories getInsuranceCategoryByid(int id)
		...
		try {
			ct = DBConnection.getConnection();
			ps = ct.prepareStatement("select * from ic_manage where id=?");
			ps.setInt(1, id);
			rs = ps.executeQuery();
			if (rs.next()) {
...
//				: new KnowledgeType(0, "可使用的资源库");
	...
	public void change_tuijian(int change_id, String select_tuijian,String table)
		...
		try {
			...
			}else if(table.equals("product_baoxian")){
				ps = ct.prepareStatement("update product_baoxian set zhengzhantuijian=? where id=?");
	...
	public List<BaoxianProduct> getSixFrontBaoxianProductList(int pageNow, int pageSize) throws ElException{
		...
		try {
			...
			sql = "select * from (select t.*,rownum rn from (select p.*,pl.lanmu from product_baoxian p " +
					" join baoxian_product_lanmu pl on p.suoshulanmu = pl.id " +
...
//				baoxianProduct.setJianjie(new OracleBlob().getContent(rs.getBlob("jianjie")));
	...
	public int getFrontBaoxianProductCount() throws ElException {
		...
		try {
			...
			sql = "select count(1) from product_baoxian p where p.shenhezhuangtai=2 and p.zhengzhantuijian='推荐' order by p.fabushijian desc";
	...
	public int getChushenCount() throws ElException {
		...
		try {
			...
			" product_baoxian pb  on p.commodityid = pb.id left join pfmsuser pf on p.createid = pf.userid " ;
	...
	public int getFrontbaoxianProductListSize(BaoxianProduct baoxianProduct,int pageNow, int pageSize)
		...
		try {
			...
			sql = "select count(1) from product_baoxian p " +
					" inner join ("+((ElNodeSQL)SpringContextUtil.getBean("elnodesql")).generateSQLByTree("baoxian_product_lanmu", baoxianProduct.getPtype(), true)+") clt on p.suoshulanmu = clt.id "+
