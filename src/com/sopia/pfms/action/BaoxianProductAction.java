...
public class BaoxianProductAction extends BaseAction {
...
//		ptypeTree = baoxianProductDao.getProTypeTree(ElConstants.TREE_ROOT,ElConstants.TREE_FIANL, true);
		if (getSessionIntValue(ElConstants.SESSION_ROLE) == 1) 
			...
			ptypeTree = baoxianProductDao.getProTypeLibTree(getSessionIntValue(ElConstants.SESSION_USERID), "op",ElConstants.TREE_FIANL, true);
		}
	...
	public String baoxianProductList() throws ElException{
		...
		if (baoxianProduct.getPtype() == null
			...
			baoxianProduct.setPtype(baoxianProductDao.getPtypeLibById(nid));
		}
		...
		System.out.println(baoxianProduct.getPtype().getId());
		
		count = baoxianProductDao.getBaoxianProductCountByUidByPerOrShar(is_baoxian_product_sh,myUserId, nid,
				 baoxianProduct.getPtype(), null,getPageNow(),getPageSize(),baoxianProduct,starttime,endtime);
		
		baoxianProductList = baoxianProductDao.getBaoxianProductByUidByPerOrShar(is_baoxian_product_sh,myUserId, nid, baoxianProduct.getPtype(),
...
//		return "success";
	...
	public String updateBaoxianProductView()throws ElException{
		...
		suoshulanmuList = baoxianProductDao.suoshulanmuList();
		baoxianProduct = baoxianProductDao.showBaoxianProduct(id);
	...
	public String updateBaoxianProduct() throws ElException{
		int roleId = baoxianProductDao.getRoleId(getSessionIntValue(ElConstants.SESSION_USERID));
		if(cptsArray != null){
...
//		}
		...
		baoxianProductDao.updateBaoxianProduct(roleId, baoxianProduct,
				baoxianProductDao.showBaoxianProduct(baoxianProduct.getId()).getShenhezhuangtai(),
	...
	public String deleteBaoxianProduct() throws ElException{
		if(baoxianProductids!=null){
			...
			for (int i = 0; i < baoxianProductidss.length; i++) {
				baoxianProductDao.deleteBaoxianProduct(Integer.parseInt(baoxianProductidss[i]));
			}
		}
	...
	public String addBaoxianProductView() throws ElException{
		if("ajax".equals(optype)){
			ptype = baoxianProductDao.getPtypeLibById(ptype.getId());
			try {
...
//			J2EEFileUtil.upload_was(logo, ext, "images/pfms/baoxianProduct", baoxianProduct.getName()+"_logo_"+id);
			...
			elUser = baoxianProductDao.getELUser(getSessionIntValue(ElConstants.SESSION_USERID));
//			baoxianProduct.setLogo(ext);
			...
			baoxianProductDao.addBaoxianProduct(is_baoxian_product_sh,baoxianProduct,elUser,ptype.getParent().getId());
//		}
		return "success";
	}
	...
	public String shenheBaoxianProduct() throws ElException{
		int roleId = baoxianProductDao.getRoleId(getSessionIntValue(ElConstants.SESSION_USERID));
		baoxianProductDao.shenheBaoxianProduct(roleId, id);
		return "success";
	}
	
	public String shenheBaoxianProductNotPass() throws ElException{
		int roleId = baoxianProductDao.getRoleId(getSessionIntValue(ElConstants.SESSION_USERID));
		baoxianProductDao.shenheBaoxianProductNotPass(roleId, id);
		return "success";
	}
	...
	public String baoxianProductType_view() throws ElException{
		...
		ptype.setOpusers(baoxianProductDao.getOpUsers( ptype
				.getId()));
		return "baoxianProductType_view";
	}
	
	public String baoxianProductType_addInit() throws ElException{
		ptypeTree = baoxianProductDao.getProTypeTree(ElConstants.TREE_ROOT,
	...
	public String baoxianProductType_add() throws ElException{
		baoxianProductDao.addProducttype(ptype);
	...
	public String baoxianProductType_alterInit() throws ElException{
		...
		ptype = baoxianProductDao.getPtypeLibById(ptype.getId());
		ptype
				.setOpusers(baoxianProductDao.getOpUsers( ptype
						.getId()));
...
//				.getId()));
	...
	public String baoxianProductType_alter() throws ElException{
		...
		baoxianProductDao.alterProductType(ptype);
	...
	public String baoxianProductType_delete() throws ElException{
		...
		if(productIsDel == 1){
			//并入上级	3	1
			baoxianProductDao.updateProductTypeParentid(ptype.getId(), ptype.getParent().getId());
			baoxianProductDao.updateProductParentid(ptype.getId(), ptype.getParent().getId());
			baoxianProductDao.deletePtype(ptype.getId());
		}else{
			//一起删除
			baoxianProductDao.deleteProductTypeAndSub(ptype.getId());
		}
//		更新新闻树左右id
//		((ElNodeSQL)SpringContextUtil.getBean("elnodesql")).updatetlrid("newstype");
	...
	public String insuranceCategory_view() throws ElException{
		IC = baoxianProductDao.getInsuranceCategoryByid(IC.getId());
		try {
			getResponse().setContentType("text/html;charset=UTF-8");
			PrintWriter localPrintWriter = getResponse().getWriter();
	...
	public String change_tuijian() throws ElException{
		if(baoxianProductids != null && !baoxianProductids.equals("")){
			...
			for(int i=0;i<ids_array.length;i++){
				baoxianProductDao.change_tuijian(Integer.parseInt(ids_array[i]),select_tuijian,"product_baoxian");
			}
		}
		return "success";
	}
