...
import com.sopia.duman.tags.DepList;
...
public class ELClassRegistration {
	...
	private List<ElClass> elclasss; // 培训班
	...
	private List<ClassPara> classParas;
	private String erParasstr;
	private String erepParasstr;
	private String classParasstr;
	private String erParasMsg;
	private String erepParasMsg;
	private String classParasMsg;
	private String myerParasMsg;
	private String myclassParasMsg;
	private String myerepParasMsg;
	private String examepids;
	public String getExamepids() {
		return examepids;
	}
	public void setExamepids(String examepids) {
		this.examepids = examepids;
	}
	...
	public String getTreeTypes()throws ElException{
		...
		if(bmName!=null){
			for(int i =0 ; i< bmName.size();i++){   
				String dep = new DepartmentDaoImpl().getByIdXiaJi(Integer.parseInt(bmName.get(i).toString()));
				if(!dep.equals("")){
					if(bm.equals("")){
						bm = bmName.get(i).toString()+","+new DepartmentDaoImpl().getByIdXiaJi(Integer.parseInt(bmName.get(i).toString())); 
					}else{					
						bm = bmName.get(i).toString()+","+bm+","+new DepartmentDaoImpl().getByIdXiaJi(Integer.parseInt(bmName.get(i).toString())); 
					}
	...
	public String getElclassids() {
		...
		if(elclasss != null){
			for (int i = 0; i < elclasss.size(); i++) {
				if (classsId.equals("")) {
					classsId = elclasss.get(i).getId() + "";
				} else {
					classsId = classsId + "," + elclasss.get(i).getId();
				}
			}
		}
		return classsId;
		...
		if(elclasss != null){
			...
			for (int i = 0; i < elclasss.size(); i++) {
				if (elclassNames.equals("")) {
					elclassNames = elclassDao.getElClassById(elclasss.get(i).getId())
							.getName();
				} else {
					elclassNames = elclassNames
							+ ","
							+ elclassDao.getElClassById(elclasss.get(i).getId())
									.getName();
				}
			}
			return elclassNames;
