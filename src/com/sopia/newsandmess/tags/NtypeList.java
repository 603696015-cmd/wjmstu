...
public class NtypeList extends ElTag{
	private static final long serialVersionUID = 3119679319963664116L;
	private static final Log logger = LogFactory.getLog(NtypeList.class);
	@SuppressWarnings("unchecked")
	public int doStartTag() {
		try {
			JspWriter out = pageContext.getOut();
			ServletRequest request = pageContext.getRequest();
			NewsType qlb = (NewsType) request.getAttribute("ntypeTree");
		...
		if(qlb!=null){
			...
			if(qlb.getLevel()==0)
				if(!getRootAble()){
					...
					out.print("d" + getDid() + ".add("+qlb.getId()+",-1,'"+qlb.getName()+"','"+href1+".shtml');\n");
				}
				else{
				...
				if(!getModel()){
					...
					out.print("d" + getDid() + ".add("+qlb.getId()+","+qlb.getParent().getId()+",'"+qlb.getName()+"','"+href1+".shtml');\n");
				}
	...
	public void writeChildsOP(JspWriter out, Object obj ,String role) throws Exception {
		...
		if(qlb.getLevel()==0)
			if(!getRootAble()){
				...
				if(role.equals("1")&&getModel()){
					out.print("d" + getDid() + ".add("+qlb.getId()+",-1,'"+qlb.getName()+"','"+href1+".shtml');\n");
				}else if(role.equals("1")&&!getModel()){
					out.print("d" + getDid() + ".add("+qlb.getId()+",-1,'"+qlb.getName()+"','"+href1+"');\n");
				}
				else{
					out.print("d" + getDid() + ".add("+qlb.getId()+",-1,'"+qlb.getName()+"');\n");
