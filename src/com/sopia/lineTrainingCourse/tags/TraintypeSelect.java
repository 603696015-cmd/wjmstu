package com.sopia.lineTrainingCourse.tags;
...
public class TraintypeSelect extends ElTag {
	...
	public int doStartTag() {
		try {
			...
			TrainTypeTree obj = (TrainTypeTree) request.getAttribute("trainTypeTree");
...
//			writeChilds(out, obj);
	...
	public void writeChilds(JspWriter out, Object obj) throws Exception {
		TrainTypeTree qlb = (TrainTypeTree) obj;
		List<TrainTypeTree> qlbChild = qlb.getChild();
		if(qlb.getId()<=0){
		...
		for (int i = 0; i < qlbChild.size(); i++) {
			TrainTypeTree qlbi = qlbChild.get(i);
