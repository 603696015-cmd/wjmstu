...
public class ExamPracRead_view extends TagSupport {
	private static final Log logger = LogFactory.getLog(ExamPracRead_view.class);
	...
	private void writeYesOrNo(JspWriter out, Question qj, int blockid)
		...
		if (qj.getStuAnswers() != null) {
			for (int i = 0; i < qj.getStuAnswers().length; i++) {
				myStuAnswer = qj.getStuAnswers()[i];
			}
		}
	...
	private void writeDazi(JspWriter out, Question qj, int blockid)
			throws Exception {
		out.println("<div> 范文： " + qj.getSubject() + "<br/> <div class='answer'><b>回答</b>：【系统打分:" + qj.getMyScore() + "】速 度："
				+ qj.getStuAnswers()[0] + "个字/秒，打对的字数：" + qj.getStuAnswers()[1] + "个， "
				+ "打字时间：" + qj.getStuAnswers()[2] + "秒。<br/><b>我打的字：</b>"
				+ qj.getStuAnswers()[3] + " </div> </div>");
	}

	private void writeMail(JspWriter out, Question qj, int blockid)
			throws Exception {
		out.println(" <div class='answer'><b>回答</b>：【系统打分:" + qj.getMyScore() + "】<br/>");
		...
		out.println(" <div style='color:green'><b>正确答案</b> 发 给：" + qj.getAnswers()[0] + "<br />" + "抄 送："
				+ qj.getAnswers()[1] + "<br />" + "密 送：" + qj.getAnswers()[2]
				+ "<br />" + "主 题：" + qj.getAnswers()[3] + "<br />" + "附 件："
				+ qj.getAnswers()[4] + "<br />" + "正 文：" + qj.getAnswers()[5]
				+ "</div> </div>");
	...
	private void writeSearch(JspWriter out, Question qj, int blockid)
		...
		out.println(" <div style='color:green'><b>正确答案</b> 关键字：" + qj.getAnswers()[0] + "<br />"
				+ "答&nbsp;&nbsp;&nbsp;案：" + qj.getAnswers()[1] + "<br />"
				+ "</div> </div>");
	...
	private void writeOffice(JspWriter out, Question qj, int blockid)
		...
		out
				.println("<div>下载： <a target='_blank' href='"
						+ qj.getSubject()
						+ "'>模板文档</a><br/>"
						+ "上传： 我的答案<input type='file' size='30' name='question.answers' ></div>");
