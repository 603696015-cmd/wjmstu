...
public class QPaper1b1 extends TagSupport {
	...
	public int doStartTag() {
		try {
			...
			MyExamPaper m = (MyExamPaper) request.getAttribute("myExamPaper");
			if(null==m)
				sqid = 0;
			else
				sqid = m.getId();
	...
	private void writeEPBs(JspWriter out, List<ExamPaperBlock> epbs)
		...
		for (int j = 0; j < epbs.size(); j++) {
			...
			out.print("<script type=\"text/javascript\">var block"
					...
					+ epbj.getSortid() + ",'"+epbj.getTitle()+"');</script>\n");
		...
		out.println("<input name=\"quesJs\" type=\"hidden\" value=\"0\" />");
	}
		...
		for (int j = 0; j < qs.size(); j++) {
			...
			out
					.print("<script type=\"text/javascript\">var question"
							...
							+ ","+_qsort+","+qj.getOpstatus()+","+qj.getQtype()+");</script>\n");
			...
			out.println("<b>第" + (qj.getSortid()) + "题</b>("
					+ epb.getEachscore() + "分)<p>" +(qj.getQtype()!=8&&qj.getQtype()!=9&&qj.getQtype()!=10?qj.getContent_():"") + "</p> ");
			...
			if (qj.getQtype() == 7) {// 材料题
				...
				for (int i = 0; i < qjc.size(); i++) {
					...
					out.println("</div>");
					out
					.print("<script type=\"text/javascript\">question"
							+ qj.getId() + ".addChild(new QUESTION(" + qjci.getId() + ","
							+ qjci.getSortid() + ",block" + epb.getId()
							+ ","+_qsort+","+qjci.getOpstatus()+","+qj.getQtype()+"));</script>\n");
	...
	private void writeYesOrNo(JspWriter out, Question qj, int blockid,
		...
		if (null == qj.getStuAnswers() || "".equals(qj.getStuAnswers()[0])) {
			out.println("<input onclick=\"q_yd("+blockid+","+qj.getId()+")\" name=\"questions[" + _qsort
					+ "].stuAnswers\" type=\"radio\" value=\"yes\" />正确");
			out.println("<input onclick=\"q_yd("+blockid+","+qj.getId()+")\" name=\"questions[" + _qsort
					+ "].stuAnswers\" type=\"radio\" value=\"no\" />错误");
			out.println("</div>");
			return;
		} else {
			if (qj.getStuAnswers()[0].equals("yes"))
				out
						.println("<input onclick=\"q_yd("+blockid+","+qj.getId()+")\" name=\"questions["
								+ _qsort
								+ "].stuAnswers\"  checked='checked' type=\"radio\" value=\"yes\" />正确");
			else
				out.println("<input onclick=\"q_yd("+blockid+","+qj.getId()+")\" name=\"questions[" + _qsort
						+ "].stuAnswers\" type=\"radio\" value=\"yes\" />正确");
			...
			if (qj.getStuAnswers()[0].equals("no"))
				out
						.println("<input onclick=\"q_yd("+blockid+","+qj.getId()+")\" name=\"questions["
								+ _qsort
								+ "].stuAnswers\"  checked='checked' type=\"radio\" value=\"no\" />错误");
			else
				out.println("<input onclick=\"q_yd("+blockid+","+qj.getId()+")\" name=\"questions[" + _qsort
						+ "].stuAnswers\" type=\"radio\" value=\"no\" />错误");
		}
	...
	private void writeEssay(JspWriter out, Question qj, int blockid, int _qsort)
		...
		if (null != qj.getStuAnswers())
			out.println("<textarea cols=80 rows=18 onkeyup=\"q_yd("+blockid+","+qj.getId()+");qsave_("+_qsort+",this);\" name=\"questions[" + _qsort
							+ "].stuAnswers\" >"
					+ qj.getStuAnswers()[0] + "</textarea></div>");
		else
			out.println("<textarea cols=80 rows=18 onkeyup=\"q_yd("+blockid+","+qj.getId()+");qsave_("+_qsort+",this);\" name=\"questions[" + _qsort
							+ "].stuAnswers\" ></textarea></div>");
		...
		out
				.println("<div>下载： <a target='_blank' href='"+SystemConfOp.getStuffUrl()+"download.jsp?filename="
						...
						+ "\" name=\"questions[" + _qsort
							+ "].stuAnswers\" />	<input onclick=\"q_yd("+blockid+","+qj.getId()+");upload_offices2( "
