...
public class QPaperAllViewNew extends TagSupport {
	private static final Log logger = LogFactory.getLog(QPaperAllViewNew.class);
	...
	private void writeEPBs(JspWriter out, List<ExamPaperBlock> epbs)
		...
		if (null != epbs)
			for (int j = 0; j < epbs.size(); j++) {
				...
				out.println(" <div ><div class='rules_list'>");
//				out.println("<div class='block_name'>");
				out.println("<div class='RulesTitle'><span class='ico'></span>" +
						ExamPaperUtil.getBaseNum2Chinese(epbj.getSortid())+"、"
						+ epbj.getTitle()+"</div>  <div class='line2'></div><div style='clear:both'></div><div >");
...
//					+ ".</div><div class='content'>");//【" + epb.getEachscore()	+ "分】
			out.println("<div class='ln-question cur exam_list' id='exam_list_2500825' " +
					"data-examno='1' data-examid='2500825' data-examtype='1'> " +
					" <div class='ln-body'><label class='exam_no'>"+qj.getSortid()+"</label>"	  );
...
//					+ "分】
				...
				if(qj.getQtype()==8){
					...
					out.println(" <div class='exam_title'>("+qj.getScore()+"分)" + qj.getContent_() + "</div>     <i class='mark'></i>  </div>");
				}
...
//			writeDazi(out, qj, blockid, qj.getSortid());
	...
	private void printQexplain(JspWriter out, Question qj) throws Exception{
		if(qj.getQexplain()!=null&&!"".equals(qj.getQexplain())){
			out.print("<dl class='exam_option' >");
			out.println("<dt>答案解析：</dt>"+ qj.getQexplain());
			out.print("</dl>");
		}
	}
...
//		out.println("<div class='select_opt'>");
		out.println(" <div class='ln-input'> <div class='ln-options ln-span1 ln-options-single'>");
		
		for (int i = 0; i < qj.getOptions().length; i++) {
			out.println("  <dl class='exam_option' >");
			out.println("<dt>" + ExamPaperUtil.getABC(i) + ":</dt>"
					+ qj.getOptions()[i] + "</dl>");
		}
//		out.print("</div>");
		out.println("<dl class='exam_option'"+(qj.getMyScore()==0?" style='color:red'":"")+">");
		out.print("<dt>标准答案：</dt>");
		if(qj.getAnswers()!=null){
			for (int i = 0; i < qj.getAnswers().length; i++) {
				out.print(ExamPaperUtil.getABC(qj.getAnswers()[i]) + "、");
			}
		}
		out.print(" </dl><dl  class='exam_option'><dt>考生答案：</dt>" + myStuAnswer+"</dl>");
...
//		out.print("<div><b>本题分数</b>："+qj.getScore()+"分</div>");
		this.printQexplain(out, qj);
		out.println("<dl  class='exam_option'><dt>批语：</dt>"+ qj.getPiyu() +"</dl>");
		out.println("</div>");
	}
