...
public class QPaper1b1View extends TagSupport {
	...
	public int doStartTag() {
		...
		try {
			...
			MyExamPaper myExamPaper = (MyExamPaper)request.getAttribute("myExamPaper");
			age = ep.getUserage();
			// epid = ep.getId();
			...
			out
					.print("<script type=\"text/javascript\">var ep = new EXAMPAPER("
							...
							+ ","
//							+ ep.getMep_tscore() + ");" +
							+ myExamPaper.getMyScore() + ");" + //TMK修改
									"var sqid_ = "+myExamPaper.getId()+";</script>\n");
			writeEPBs(out, epbs);
			out
...
//			}
	...
	private void writeEPBs(JspWriter out, List<ExamPaperBlock> epbs)
		...
		for (int j = 0; j < epbs.size(); j++) {
			...
			if(epbj.getType()!=15&&epbj.getType()!=16&&epbj.getType()!=17&&epbj.getType()!=18&&epbj.getType()!=19&&epbj.getType()!=20){
	...
	private int writeEPQs(JspWriter out, List<Question> qs, ExamPaperBlock epb,
		...
		for (int j = 0; j < qs.size(); j++) {
			...
			out.println("<div class='question' id='question_" + epb.getId()
					+ "_" + qj.getId() + "'>");
			out.println("<input type=\"hidden\" id='qtype_"+epb.getId()+"_"+qj.getId()+"' value="+qj.getQtype()+" />");
...
//				out.println("<p><div name='dazicontent'>" + newdaziContent.toString() + "</div></p> ");
				...
				if(qj.getQtype()!=15&&qj.getQtype()!=16&&qj.getQtype()!=17&&qj.getQtype()!=18&&qj.getQtype()!=19&&qj.getQtype()!=20){
