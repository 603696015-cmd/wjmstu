...
public class QPaper2 extends TagSupport {
	private static final Log logger = LogFactory.getLog(QPaper.class);
	...
	public int doStartTag() {
		try {
			...
			userid = (Integer) ((HttpServletRequest)request).getSession().getAttribute(ElConstants.SESSION_USERID);
//			epid = ep.getId();
			List<ExamPaperBlock> epbs = ep.getEpBlocks();
			writeEPBs(out, epbs);
		} catch (Exception ex) {
			logger.error("试卷显示错误",ex);
		}
	...
	private void writeEPQs(JspWriter out, List<Question> qs, ExamPaperBlock epb)
		...
		for (int j = 0; j < qs.size(); j++) {
			...
			//--------调查问卷加
			int sortid = qj.getSortid();
			qj = qdao.getQbyId(qj.getId());
			qj.setSortid(sortid);
			//---------------
	...
	private void writeSearch(JspWriter out, Question qj, int blockid)
		...
		out
				.println("关键字：" + "<input type=\"text\" name=\""
						...
						+ "<br />"
						+ "答&nbsp;&nbsp;&nbsp;案："
						+ "<input type=\"text\" id=\"qanswer_ans_"
						+ blockid
						+ "_"
						+ qj.getId()
						+ "\" name=\""
		...
		out.println("<script type=\"text/javascript\">" + "qsearch_" + blockid
				+ "_" + qj.getId() + "=new Qsearch ('" + "_" + blockid + "_"
				+ qj.getId() + "');" + "</script>");
	...
	private void writeOffice(JspWriter out, Question qj, int blockid)
		...
		out
				.println("<div>下载： <a target='_blank' href='"+SystemConfOp.getStuffUrl()+"download.jsp?filename="
						...
						+ "_"
						+ qj.getId()
						+ "' size='30' name='st' ><input type=\"hidden\" id='a_office_"+ blockid
						+ "_"
						+ qj.getId()
						+"' value=\""+userid+"/"
						+ blockid
						...
						+ "_"
						+ qj.getSortid()
						+ "\" size=\"45\" name=\"question.answers\" />	<input type='button' value='上傳' onclick=\"upload_offices( "
