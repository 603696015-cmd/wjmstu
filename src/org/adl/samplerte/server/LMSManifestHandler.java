...
public class LMSManifestHandler extends CPDOMParser implements Serializable {
	...
	/***************************************************************************
	 * * * Method: LMSManifestHandler() * Input: None * * Output: None * *
	...
	/***************************************************************************
	 * * * Method: processManifest() * Input: None * Output: boolean - Whether
	 * or not the method was successful * Description: This method initiates the
	 * RTE import/parsing of a manifest * file from a PIF content package. * *
	 * Side Effects: none *
	 **************************************************************************/
	public boolean processManifest() {
		...
		if (DebugIndicator.ON) {
			System.out
					.println("Entering LMSManifestHandler::processManifest()");
		}
	...
	/***************************************************************************
	 * * * Method: processContent() * Input: none * Output: boolean - Whether
	 * the method was successful or not. * Description: This method starts at
	 * the root node and then calls * calls fillManifest which populates the
	 * parser structure. * * Side Effects: none *
	 **************************************************************************/
	...
	/***************************************************************************
	 * * * Method: processResources * Input: Node resourcesNode - The
	 * <resources> node from the imsmanfest file * Output: * Description: This
	 * method populates the resources section of the parser * structure. * *
	 * Side Effects: none *
	 **************************************************************************/
	...
	/***************************************************************************
	 * * * Method: processResource() * Input: Node resourceNode - A <resource>
	 * node from the imsmanifest file * Output: * Description: This method
	 * populates the resource section of the parser * object. * * Side Effects:
	 * none *
	 **************************************************************************/
	...
	/***************************************************************************
	 * * * Method: updateItemList * Input: String id - the identifier of the
	 * item * String scormType - the scormtype of the item (or reference) *
	 * String type - the type attribute * String href - the href location *
	 * Output: * Description: This method adds the launch location to the item
	 * vector. * The launch is not part of the <item> element, so this * method
	 * compares the identifier and identifierref and when * a match is found,
	 * the item section of the parser object * is populated with the resource
	 * launch location. This is * done only for convenience. * * Side Effects:
	 * none *
	 **************************************************************************/
	...
	/***************************************************************************
	 * * * Method: getCourseID() * Input: none * Output: String - this course
	 * id. * Description: This is a getter method on the courseID member. *
	 **************************************************************************/
	...
	/***************************************************************************
	 * * * Method: getOrgsCopy() * Input: none * Output: ADLOrganizations *
	 * Description: This method was developed anticipating sequencing for *
	 * version 1.3. This method will return the entire course * <organization>
	 * information. This will include all * sequencing information. *
	 **************************************************************************/
	...
	/***************************************************************************
	 * * * Method: setSequence() * Input: none * Output: none * Description:
	 * This method will be used to set the control type when * sequencing is
	...
	/***************************************************************************
	 * * * Method: updateDB() * Input: none * Output: none * Description: This
	 * method takes the relevant information from the * populated parser
	 * structure and writes it to a related * database. This is done so that the
	 * JSP coding is * more straight forward. * * Side Effects: none *
	 **************************************************************************/
...
//			String connectionURL = "jdbc:odbc:SampleRTE";
			...
			String sqlSelectCourseID = "SELECT * FROM SC_ApplicationData WHERE dataName = ?";

			String sqlUpdateCourseID = "UPDATE SC_ApplicationData SET numberValue = ? WHERE dataName = ?";

			String sqlInsertCourse = "INSERT INTO SC_CourseInfo (CourseID, CourseTitle, Active, Control) "
					+ "VALUES(?, ?, ?, ?)";

			String sqlInsertItem = "INSERT INTO SC_ItemInfo (CourseID, Identifier, Type, Title, Launch, "
...
//			conn = DriverManager.getConnection(connectionURL);
			...
			if (selectCourseIDRS.next()) {
				...
				// courseDir =
				// "C:\\jakarta-tomcat-4.0\\webapps\\adl\\CourseImports\\" +
				// courseID;
				...
				for (int i = 0; i < items.size(); i++) {
					...
					synchronized (stmtInsertItem) {
						...
						stmtInsertItem.setString(6, tempItem
								.getParameterString());
						stmtInsertItem.setString(7, tempItem.getDataFromLMS());
						stmtInsertItem
								.setString(8, tempItem.getPrerequisites());
						stmtInsertItem.setString(9, tempItem.getMasteryScore());
						stmtInsertItem.setString(10, tempItem
								.getMaxTimeAllowed());
						stmtInsertItem.setString(11, tempItem
								.getTimeLimitAction());
	...
	/***************************************************************************
	 * * * Method: getAttribute() * Input: Node theNode - The current node that
	 * was traversed to * String the Attribute - The name of the attribute that
	 * the value * is desired for. * Output: * Description: This method * * Side
	 * Effects: none *
	 **************************************************************************/
	 ...
	 * attributes * Output: Attr[] - Sorted array of attributes * Description:
	 * This method returns an array of sorted attributes. * * Side Effects: none *
	 **************************************************************************/
	...
	/***************************************************************************
	 * * * Method: getSubElement() * Input: Node node - The current node that
	 * has been traversed to * String element - The name of the sub-element
	 * that's value is * desired. * Output: String - The value of the
	 * sub-element * Description: This method gets the value of a desired
	 * sub-element. * Side Effects: *
	 **************************************************************************/
	...
	/***************************************************************************
	 * * * Method: getText() * Input: Node node - The current node * Output:
	 * String - The text of the desired node. * Description: This method gets
	 * the child text node of the node that is * passed in for input. * Side
	 * Effects: *
	 **************************************************************************/
