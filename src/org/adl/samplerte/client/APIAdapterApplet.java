   ...
   ***************************************************************************/
   ...
   public void init()
   {
      ...
      try
      {
    	 ...
    	 courseid = this.getParameter("COURSEID");
    	 classid = this.getParameter("CLASSID");
    	 cid = this.getParameter("CID");
    	 scoid = this.getParameter("SCOID");
    	 userid = this.getParameter("USERID");
    	 //alert(servleturl+"---"+courseid+"---"+classid+"---"+scoid+"---"+userid);
         this.servletURL = new URL(servleturl);//protocol + "://" + host + ":" + port + "/adl/lmscmi"
...
//         alert(this.servletURL.toString());
      }
      catch ( Exception e )
      {
    	  alert("API≥ı ºªØ“Ï≥£"+e.toString());
         e.printStackTrace();
   ...
   public String LMSInitialize(String param)
   {
      ...
      else
      {
         ...
         ServletProxy servletProxy = 
        	 new ServletProxy(this.servletURL,this.userid,this.scoid,this.courseid,this.classid,this.cid,sessionid);//new ServletProxy(this.servletURL, sessionid);
   ...
   ***************************************************************************/
   public String LMSCommit(String param)
   {
      ...
      if ( (tempParm.equals("null") || tempParm.equals("")) == true )
      {
         ...
         String sessionid = getCookie("JSESSIONID");
         ServletProxy servletProxy = new ServletProxy(this.servletURL,this.userid,this.scoid,this.courseid,this.classid,this.cid,sessionid);//new ServletProxy(this.servletURL, sessionid);
   ...
   ***************************************************************************/
   ...
   public void setScoid(String scoid) {
	   if ( _Debug )
	      {
	         System.out.println("In API::setScoid():"+scoid);
	      }
	   this.scoid = scoid;
	   }
}
